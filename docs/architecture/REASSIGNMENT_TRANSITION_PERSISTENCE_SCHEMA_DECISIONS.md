...
**Related:** `CONTROLLED_PRE_START_REASSIGNMENT_DECISIONS.md` Â· `CONTROLLED_PRE_START_REASSIGNMENT_IMPLEMENTATION_READINESS.md`
...
| Preserve when present | `assignment_actor_user_id` â†’ `actor_user_id`; `assignment_updated_at` â†’ `created_at`; `assignment_source` â†’ optional `source` / provenance note |
| Missing metadata | `actor_user_id = null`; use exact available timestamp only; never invent actor/time |
| Fixture expectation | Exactly one backfill row for LEDâ†’7 on plan 23 |
...
| Codes | `EMPLOYEE_UNAVAILABLE` Â· `EMPLOYEE_INACTIVE` Â· `ELIGIBILITY_CHANGED` Â· `INCORRECT_INITIAL_ASSIGNMENT` Â· `MANAGER_CORRECTION` Â· `PLANNING_CHANGE` Â· `OPERATIONAL_REBALANCE_PRE_START` Â· `OTHER` |
...
| Success lost response | Retry same `transition_id` â†’ prior success |

Payload fingerprint for â€œsame payloadâ€ (conceptual): `transition_type`, `execution_plan_id`, `task_key`, `expected_current_employee_id`, `previous_employee_id`/`new_employee_id` as applicable, `reason_code`, normalized `reason_note`.
...
Failure of either write â†’ **ROLLBACK BOTH**.
...
| `execution_plan_id` | INTEGER | NO | FK â†’ `execution_plan.id` |
...
| `previous_employee_id` | INTEGER | YES | Null on first ASSIGN / after unassignâ†’assign |
...
6. Re-run backfill â†’ zero new rows (idempotent).
7. Verify `tasks_json` SHA / `updated_at` / embedded assignee **unchanged**.
8. Verify protected fixtures (880750 LEDâ†’7; baselines).
9. On inconsistency â†’ **STOP**; no silent repair.
...
Mismatch â†’ `ASSIGNMENT_TRANSITION_STATE_MISMATCH`.
...
execution.task_unassign  â†’ admin, manager
```
...
Only **`CLEAR`** allows reassign/unassign. Unknown/not configured â†’ **blocked** (fail-closed). Absence of a scheduling system â‰  CLEAR.
...
Any session/execution history for the task â†’ reassign and unassign **blocked**. Active-only check is insufficient. Post-start operational transfer = future Phase E architecture.
