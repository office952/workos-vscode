# Reassignment Transition Persistence Schema ΓÇö Owner Decisions
...
| Production dialect | Not assumed SQLite ΓÇö Alembic + SQLAlchemy async; tests mention `postgresql`/`asyncpg`; design must be dialect-portable |
| Current assignee store | `execution_plan.tasks_json` ΓåÆ `operational_tasks[].assigned_employee_id` |
| Plan PK | `execution_plan.id` INTEGER |
| Plan `order_id` | INTEGER NOT NULL ΓÇö **no FK** to orders in live pragma (denormalized identity, same pattern as `execution_task_participants.order_id`) |
| Employees PK | `employees.id` INTEGER |
| Users PK | `users.id` VARCHAR(255) ΓÇö matches embedded `assignment_actor_user_id` shape |
...
| Permission matrix | Declarative Python `PERMISSION_MATRIX` in `dependencies/permissions.py` ΓÇö `execution.task_assign` includes operator; reassign/unassign keys **absent** |
...
## DEC-REASSIGN-SCHEMA-01 ΓÇö Persistence model
...
## DEC-REASSIGN-SCHEMA-02 ΓÇö Backfill
...
| Preserve when present | `assignment_actor_user_id` ΓåÆ `actor_user_id`; `assignment_updated_at` ΓåÆ `created_at`; `assignment_source` ΓåÆ optional `source` / provenance note |
| Missing metadata | `actor_user_id = null`; use exact available timestamp only; never invent actor/time |
| Fixture expectation | Exactly one backfill row for LEDΓåÆ7 on plan 23 |

---

## DEC-REASSIGN-SCHEMA-03 ΓÇö Transition identity
...
## DEC-REASSIGN-SCHEMA-04 ΓÇö Uniqueness

| Field | Value |
| ----- | ----- |
| Decision | **`UNIQUE_TRANSITION_ID`** |
| Required | `UNIQUE(transition_id)` global |
| Evaluated optional | `UNIQUE(execution_plan_id, task_key, transition_id)` ΓÇö redundant if `transition_id` is globally unique UUID; optional defense-in-depth only |
| Forbidden | Unique on `task_key` alone; unique on employee alone |

---

## DEC-REASSIGN-SCHEMA-05 ΓÇö Current-state authority
...
| Consistency | Latest transition `new_employee_id` must match embedded assignee; mismatch ΓåÆ fail-closed `ASSIGNMENT_TRANSITION_STATE_MISMATCH` ΓÇö **no auto-heal** |

---

## DEC-REASSIGN-SCHEMA-06 ΓÇö Foreign keys
...
| --------- | ----------------------------- | --------- |
| `execution_plan_id` ΓåÆ `execution_plan.id` | FK `ON DELETE RESTRICT` | Plan deletion must not orphan transitions silently |
...
| `previous_employee_id` / `new_employee_id` ΓåÆ `employees.id` | Prefer **FK `ON DELETE RESTRICT`** for active integrity; alternative **nullable columns without FK** if hard-delete of employees is operationally required | History must survive inactive/deleted employees ΓÇö soft-delete/`status`/`end_date` preferred over hard delete |
| `actor_user_id` ΓåÆ `users.id` | FK optional; if present use `ON DELETE SET NULL` (VARCHAR identity) | Actor may be removed; retain transition with null actor rather than delete row |
| `task_key` | String, **no task table FK** | Tasks live in embedded JSON |

Contrast: `execution_task_participants.employee_id` uses `ON DELETE CASCADE` ΓÇö **must not** be copied for transition history.

---

## DEC-REASSIGN-SCHEMA-07 ΓÇö Append-only
...
## DEC-REASSIGN-SCHEMA-08 ΓÇö Reason and note
...
## DEC-REASSIGN-SCHEMA-09 ΓÇö Ordering
...
## DEC-REASSIGN-SCHEMA-10 ΓÇö Retry semantics
...
| Success lost response | Retry same `transition_id` ΓåÆ prior success |

Payload fingerprint for ΓÇ£same payloadΓÇ¥ (conceptual): `transition_type`, `execution_plan_id`, `task_key`, `expected_current_employee_id`, `previous_employee_id`/`new_employee_id` as applicable, `reason_code`, normalized `reason_note`.

---

## DEC-REASSIGN-SCHEMA-11 ΓÇö Consistency check
...
## DEC-REASSIGN-SCHEMA-12 ΓÇö Migration safety
...
| Phases | A schema/backfill ┬╖ B commands ┬╖ C QA proof ΓÇö each needs separate Owner GO |
...
Failure of either write ΓåÆ **ROLLBACK BOTH**.  
...
## Proposed table (conceptual ΓÇö not created)
...
| `execution_plan_id` | INTEGER | NO | FK ΓåÆ `execution_plan.id` |
...
| `previous_employee_id` | INTEGER | YES | Null on first ASSIGN / after unassignΓåÆassign |
...
| `metadata_json` | TEXT/JSON | YES | Optional compact non-PII bag ΓÇö never full `tasks_json` / employee / JWT |
| `created_at` | TIMESTAMP WITH TIME ZONE | NO | From request time or preserved backfill timestamp |

\*Backfill `ASSIGN` rows: reason may be null or a dedicated synthetic code such as `LEGACY_BACKFILL_INITIAL_ASSIGN` ΓÇö **must be decided in Phase A** without inventing fake operational reasons. Prefer explicit synthetic code over pretending `MANAGER_CORRECTION`.
...
Prefer application-level string enums (OpenAPI + Python) for SQLite/Postgres portability. Optional native Postgres ENUM only if production dialect is confirmed Postgres **and** Owner accepts Alembic complexity ΓÇö default recommendation: **VARCHAR + CHECK**.
...
6. Re-run backfill ΓåÆ zero new rows (idempotent).  
7. Verify `tasks_json` SHA / `updated_at` / embedded assignee **unchanged**.  
8. Verify protected fixtures (880750 LEDΓåÆ7; baselines).  
9. On inconsistency ΓåÆ **STOP**; no silent repair.  
...
Mismatch ΓåÆ `ASSIGNMENT_TRANSITION_STATE_MISMATCH`.
...
## Permissions (retained targets ΓÇö not implemented)

```text
execution.task_reassign  ΓåÆ admin, manager
execution.task_unassign  ΓåÆ admin, manager
```
...
Only **`CLEAR`** allows reassign/unassign. Unknown/not configured ΓåÆ **blocked** (fail-closed). Absence of a scheduling system Γëá CLEAR.
...
Any session/execution history for the task ΓåÆ reassign and unassign **blocked**. Active-only check is insufficient. Post-start operational transfer = future Phase E architecture.
...
| Concern | Guidance |
| ------- | -------- |
| Local QA | SQLite ΓÇö INTEGER PK, CHECK, UNIQUE supported |
...
## Test strategy (future ΓÇö not executed here)
...
## Dead pieces (record only ΓÇö nothing removed)
