"""
Seed isolated s67 DB with MachineRun states for shop-floor UI screenshots.
...
DB_PATH = OUT_DIR / "_isolated_s67_ui.db"
...
def _window(offset_hours: int):
    start = datetime(2026, 8, 9, 8, 0, tzinfo=timezone.utc) + timedelta(hours=offset_hours)
...
async def main() -> None:
    ...
    async with factory() as session:
        ...
        reserved = await _create(session, 3, 23, FACE_C, 24, FACE_D)
        reserved = await confirm_machine_run(
            session,
            machine_run_id=reserved.machine_run_id,
        ...
        await session.commit()
        ids["RESERVED"] = reserved.machine_run_id
        ...
        completed = await _create(session, 9, 27, FACE_C, 28, FACE_D)
        completed = await confirm_machine_run(
            session,
        ...
        await session.commit()
        completed = await start_machine_run(
            session,
            machine_run_id=completed.machine_run_id,
            command=StartMachineRunCommand(
                expected_version=completed.version,
        ...
        await session.commit()
        ids["COMPLETED"] = completed.machine_run_id

        released = await _create(session, 12, 29, FACE_A, 30, FACE_B)
        released = await confirm_machine_run(
            session,
            machine_run_id=released.machine_run_id,
            command=ConfirmMachineRunCommand(
        ...
        await session.commit()
        released = await start_machine_run(
            session,
            machine_run_id=released.machine_run_id,
            command=StartMachineRunCommand(
                expected_version=released.version,
        ...
        await session.commit()
        released = await complete_machine_run(
            session,
            machine_run_id=released.machine_run_id,
            command=CompleteMachineRunCommand(
                expected_version=released.version,
        ...
        await session.commit()
        ids["RELEASED"] = released.machine_run_id
    ...
    (OUT_DIR / "_isolated_state_ids.json").write_text(
