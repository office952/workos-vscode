/**
 * Slice 2 screenshot capture — Intake V6 badge noise reduction.
 */
...
async function main() {
  ...
  await shot(page, "01_step1_reduced_completion_noise");

  const warnings = page.getByTestId("intake-v6-layers-warnings");
  if ((await warnings.count()) > 0) {
    await warnings.scrollIntoViewIfNeeded();
    await page.waitForTimeout(500);
  }
  await shot(page, "02_step1_analysis_summary");
  ...
  await page.evaluate(() => window.scrollTo(0, 0));
  await shot(page, "03_step2_finisaje_reduced_badges");

  await page.getByTestId("intake-v6-review-tab-iluminare").click();
  await page.waitForTimeout(1000);
  await page.evaluate(() => window.scrollTo(0, 0));
  await shot(page, "04_step2_iluminare_no_on_pill");
  ...
  await shot(page, "05_step2_diagnostic_still_available");

  await page.getByTestId("intake-v6-progress-step-confirm").click();
  await page.waitForTimeout(2000);
  await page.evaluate(() => window.scrollTo(0, 0));
  await shot(page, "06_step3_no_intentional_changes");
