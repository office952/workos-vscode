"""W7-T02 Scenario C — disposable Letters walk with measurable minute variance.

Build 1 order 92402 remains untouched (Scenarios A+B read-only).
Uses the same DETERMINISTIC_LOCAL_SCENARIO clone pattern as Build 1.
"""
...
OUT = Path(__file__).resolve().parent / "scenario_c_variance_evidence.json"
HEADERS = {
...
def main() -> int:
    ...
    ir = f"IR-W7T02-{ts}"
    evidence: dict = {
        "label": "W7-T02 SCENARIO C MINUTE VARIANCE DISPOSABLE",
        "base": BASE,
        "started_at": datetime.now(timezone.utc).isoformat(),
        "steps": [],
        "ids": {"intake_request_code": ir, "build1_order_untouched": 92402},
        "scenario": "C_measurable_minute_variance",
    }
    ...
    with httpx.Client(base_url=BASE, headers=HEADERS, timeout=180.0) as client:
        ...
        _step(
            ...
            client.post(
                ...
                json={
                    "code": ir,
                    "client_id": 1,
                    "client_name": "W7T02 Variance Letters",
                    "contact_person": "W7T02",
                    "channel": "email",
                    "product_family": "litere_volumetrice",
                    "description": "W7-T02 scenario C minute variance disposable",
                    "dimensions": "—",
                    "quantity": 1,
                    "status": "new",
                    "assigned_to": "—",
                    "notes": "disposable w7t02 variance",
                    "priority": "normal",
        ...
        evidence["steps"].append(
            {
                "step": "clone_payload_sql",
                "status": 200,
                "ok": True,
                "body": {"workspace_id": ws, "qualification": "DETERMINISTIC_LOCAL_SCENARIO"},
            }
        )
        ...
        # Freeze capture before sessions
        freeze_before = {
            "quote_id": quote_id,
            "snapshot_code": evidence["ids"].get("snapshot_code"),
            "snapshot_frozen_total_gross": evidence["ids"].get("snapshot_frozen_total_gross"),
            "order_id": order_id,
        }
        plan_get = _step(
            evidence,
            "plan_before_actuals",
            client.get(f"/api/v1/execution/plan/{order_id}"),
        )
        if not isinstance(plan_get, dict) or plan_get.get("detail"):
            plan_get = _step(
                evidence,
                "plan_v2_before",
                client.get(f"/api/v1/execution/plan-v2/{order_id}"),
            )
        freeze_before["plan_id"] = plan_get.get("id") or plan_get.get("plan_id")
        freeze_before["plan_tasks_hash"] = hash(json.dumps(plan_get.get("tasks") or plan_get.get("tasks_json") or plan_get, default=str)[:4000])
        evidence["freeze_before"] = freeze_before

        if task_id:
            # Force 75-minute closed session for measurable variance vs planned minutes
            start_ts = datetime(2026, 7, 17, 8, 0, 0, tzinfo=timezone.utc)
            end_ts = start_ts + timedelta(minutes=75)
            _step(
                evidence,
                "reality_start",
                client.post(
        ...
        evidence["stock_gap_explicit"] = True  # G3 not forced (W7-T02 exclusion)

        pj = _step(evidence, "post_job_truth", client.get(f"/api/v1/execution/{order_id}/post-job-truth"))
        ops = (pj.get("reconciliation") or {}).get("operations") or []
        summary = (pj.get("reconciliation") or {}).get("summary") or {}
        target = next((o for o in ops if o.get("task_id") == task_id), None)
        evidence["ids"]["post_job_summary"] = summary
        evidence["ids"]["target_operation"] = {
            "task_id": task_id,
            "reconciliation_state": (target or {}).get("reconciliation_state"),
            "planned_minutes": (target or {}).get("planned_minutes"),
            "actual_minutes": (target or {}).get("actual_minutes"),
            "variance_minutes": (target or {}).get("variance_minutes"),
        }
        evidence["scenario_c_pass"] = (
            (target or {}).get("reconciliation_state") == "variance"
            or (
                isinstance(((target or {}).get("variance_minutes") or {}).get("value"), (int, float))
                and abs(float(((target or {}).get("variance_minutes") or {}).get("value"))) > 0
            )
        )
        ...
        evidence["freeze_after"] = {
            "snapshot_frozen_total_gross": after,
            "plan_id": plan_after.get("id") or plan_after.get("plan_id") or freeze_before.get("plan_id"),
            "plan_tasks_hash": hash(
                json.dumps(plan_after.get("tasks") or plan_after.get("tasks_json") or plan_after, default=str)[:4000]
            ),
        }
        evidence["freeze_immutability"] = {
            "commercial_total_before": before,
            "commercial_total_after": after,
            "commercial_unchanged": before == after,
            "plan_hash_unchanged": freeze_before.get("plan_tasks_hash")
            == evidence["freeze_after"].get("plan_tasks_hash"),
            "not_build1_92402": order_id != 92402,
        }
        evidence["lineage"] = {
            "intake_request_code": ir,
            "workspace_id": ws,
            ...
            "snapshot_code": evidence["ids"].get("snapshot_code"),
            "order_id": order_id,
            "plan_id": freeze_before.get("plan_id"),
            "task_id": task_id,
            "not_23099": order_id != 23099,
            "not_92402": order_id != 92402,
        }
        ...
        evidence["verdict"] = (
            "WALK_PASS"
            if critical_ok
            and evidence["freeze_immutability"]["commercial_unchanged"]
            and evidence.get("scenario_c_pass")
            and order_id != 92402
            else "WALK_PARTIAL"
        )
