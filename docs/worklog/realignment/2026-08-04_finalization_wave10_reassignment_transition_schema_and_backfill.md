...
880811 plan 22 / 973019 plan 21 unchanged vs backup.
...
| --- | ------ | ------- | ---- | ------- |
| bb4c466 | d393b27f | Phase A schema/backfill implementation | code+migration+tests | YES |
...
1. Pre-Wave-10 / pre-app-reload with new model: no execution_task_assignment_transitions; no lembic_version (create_all-era QA DB).
2. After bb4c466 while writers still up: ORM create_all created **empty** transition table (0 rows) with ORM-named indexes only. Backup proves table present, count=0, no lembic_version.
3. Writers stopped (Owner-authorized): stop-dev.ps1 + stop :8002.
4. Backup copied to _qa_backups/wave10_phase_a/dev.db.before_s63_20260804_225424 (SHA prefix 231f3d1e).
5. **Not** lembic upgrade head CLI. Controlled script called **s63.upgrade()** via MigrationContext + Operations on sync SQLite.
...
8. Migration indexes added (ix_exec_task_assign_tr_* including composite plan/task/id). ORM indexes retained -> duplicate index names on same columns.
9. Backfill ran inside upgrade() -> 7 ASSIGN rows; consistency fail-closed check inside upgrade() passed.
10. Stamp: same script created lembic_version and set s63_execution_task_assignment_transitions **after** upgrade body (SQL insert/update - not lembic stamp CLI).
11. Idempotent re-invoke of upgrade() observed **7 -> 7**, 	asks_json unchanged.
...
Why no create collision? _table_exists -> skip create_table
...
### Schema parity (model <-> s63 <-> live SQLite)
...
Columns, nullability, PK, UNIQUE(	ransition_id), CHECK types, CHECK employee shape, FKs plan/employees ON DELETE RESTRICT, no actor FK - match across model, migration DDL, and live sqlite_master.
Live DB has both ORM-named and migration-named indexes (superset). Not repaired in evidence closure.
