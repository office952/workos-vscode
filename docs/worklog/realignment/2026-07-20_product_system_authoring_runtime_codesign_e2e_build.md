...
| Planificat | **RENAME+MOVE+DEMOTE** → „În dezvoltare” secondary nav cluster (sections non-functional; badge kept demoted) |
| Publică fail-closed | FE `resolvePublishUiGate` — disabled when readiness BLOCKED even if publication GET `publish_allowed=true` |
