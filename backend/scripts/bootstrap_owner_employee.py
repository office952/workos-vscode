"""Link owner User → Employee for Employee Mobile self identity (idempotent, env-driven).
...
Usage (PowerShell):
    ...
    python backend/scripts/bootstrap_owner_employee.py

Real run: set WORKOS_OWNER_BOOTSTRAP_DRY_RUN='0'
"""
...
async def run() -> dict:
    ...
    if not db_manager.async_session_maker:
        raise RuntimeError("bootstrap_owner_employee: database not initialized")

    config = load_bootstrap_config_from_env()
    async with db_manager.async_session_maker() as session:
        result = await bootstrap_owner_employee(session, config)
