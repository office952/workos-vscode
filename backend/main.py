...
from core.startup_safety import run_startup_safety_checks, log_startup_safety_report
# MODULE_IMPORTS_END
...
async def lifespan(app: FastAPI):
    ...
    # BUILD 20: Run environment safety checks before other initialization
    safety_report = run_startup_safety_checks()
    log_startup_safety_report(safety_report)

    if safety_report.overall_status == "BLOCKED":
        raise RuntimeError(
            f"Startup blocked by environment safety checks for {safety_report.environment}"
        )
...
@app.middleware("http")
async def csrf_report_only_header_middleware(request: Request, call_next):
    """Attach CSRF report-only status to response headers when available."""
    response = await call_next(request)
    report_status = getattr(request.state, "csrf_report_only", None)
    report_reasons = getattr(request.state, "csrf_report_only_reasons", None)
    if report_status:
        header_value = report_status
        if report_reasons:
            header_value = f"{report_status}:{','.join(report_reasons)}"
...
# Development defaults permit localhost ports. Production MUST set explicit origins.
# Wildcard with credentials is NEVER permitted.
_default_origins = [
    "http://localhost:5173",
    "http://localhost:3000",
    "http://localhost:3001",
...
# Defensive guard: wildcard origins are invalid/insecure with credentials.
...
def frontend_runtime_config():
    """Read-only runtime config for frontend bootstrap.

    Call sites append `/api/v1/...`, so this returns host-root API base.
    """
    return {"apiBaseUrl": ""}
