...
async def get_object_info(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid storage object key")
    except ValueError as e:
        logger.error(f"Invalid get object metadata request: {e}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
...
async def rename_object(request: RenameRequest, _current_user: UserResponse = Depends(require_permission("storage.rename"))):
    """
...
async def delete_object(request: ObjectRequest, _current_user: UserResponse = Depends(require_permission("storage.delete"))):
    """
...
async def upload_file(request: FileUpDownRequest, _current_user: UserResponse = Depends(require_permission("storage.upload_url"))):
    """
...
async def download_file(request: FileUpDownRequest, _current_user: UserResponse = Depends(require_permission("storage.download_url"))):
    """
