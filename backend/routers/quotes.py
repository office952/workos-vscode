...
from services.legacy_quote_price_retirement import raise_legacy_quote_price_retired
...
@router.post("/price", include_in_schema=False)
async def price_quote(
    payload: QuotePriceRequest,
    db: AsyncSession = Depends(get_db),
    _user=Depends(require_permission("quote.price")),
):
    """RETIRED — legacy CostEngine / QuoteOrchestrator customer pricing.

    Active commercial authority: Intake V6 → CommercialPriceProposal 7G.
    No calculation and no financial write are performed.
    """
    logger.info(
        "POST /api/v1/entities/quotes/price rejected — legacy_quote_price_retired"
    )
    raise_legacy_quote_price_retired()


@router.post("/{quote_id}/price", include_in_schema=False)
async def price_existing_draft_quote(
    ...
    """RETIRED — legacy in-place / revision pricing via QuoteOrchestrator.

    Active commercial authority: Intake V6 → CommercialPriceProposal 7G.
    No calculation and no financial write are performed.
    """
    logger.info(
        "POST /api/v1/entities/quotes/%s/price rejected — legacy_quote_price_retired",
        quote_id,
    )
    raise_legacy_quote_price_retired()
