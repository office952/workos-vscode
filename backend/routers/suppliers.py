...
router = APIRouter(
    prefix="/api/v1/entities/suppliers",
    tags=["suppliers"],
...
async def create_suppliers(
    data: SuppliersData,
    db: AsyncSession = Depends(get_db),
    _user: UserResponse = Depends(require_permission("supplier.create")),
):
...
async def update_supplierss_batch(
    ...
    db: AsyncSession = Depends(get_db),
    _user: UserResponse = Depends(require_permission("supplier.update")),
):
...
async def delete_supplierss_batch(
    ...
    db: AsyncSession = Depends(get_db),
    _user: UserResponse = Depends(require_permission("supplier.delete")),
):
