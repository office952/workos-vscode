...
def _layers_by_key(layers: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for layer in layers:
        key = _text(layer.get("layer_key") or layer.get("layer_id"))
        if key:
            indexed[key] = layer
    return indexed
    ...
    *,
    ...
    for item in items:
        ...
        for raw_layer_key in _as_list(item.get("source_layer_ids")):
            ...
            if not layer_key or layer_key in seen_in_confirmation:
                continue
            ...
            if role not in LOGO_ROLES and not _is_logoish(layer_key) and not _is_logoish(layer.get("layer_name")):
                continue
            seen_in_confirmation.add(layer_key)
            logo_bindings_by_key[layer_key] = _logo_binding_row(
                layer_key=layer_key,
                layer=layer,
                target_template_code=target_template_code,
            )
