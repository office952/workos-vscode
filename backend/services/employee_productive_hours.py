...
async def compute_employee_productive_hours(
    ...
    *,
    ...
    return {
        ...
        "source": "company_calendar_minus_approved_leave",
    }
...
async def compute_productive_hours_by_employee(
    ...
    *,
    ...
    """Batch productive hours for CostEngine aggregation."""
    ids = list(employee_ids)
    workdays = company_workdays_in_month(year, month)
    workday_set = set(workdays)
    baseline = float(len(workdays)) * WORK_HOURS_PER_DAY
    if not ids:
        return {}
    ...
    # Employees with no leave still get baseline (explicit for clarity).
    for emp_id in ids:
        out.setdefault(emp_id, baseline)
    return out
