...
PILOT_WRITABLE_PATHS = [
    ...
    f"{_FINISH}.mounting_solution",
    f"{_FINISH}.mains_cable_length_m",
    f"{_FINISH}.power_supply_service_corner",
    f"{_FINISH}.service_screw_finish",
]
...
VOLUMETRIC_FIELD_BINDINGS: list[IntakeFormFieldBinding] = [
    ...
    IntakeFormFieldBinding(
        canonical_key="mounting_solution",
        workspace_path=f"{_FINISH}.mounting_solution",
        label_ro="Soluție suport",
        required=False,
        field_type="object",
        decision="operator_select",
        consumers=["structura_suport", "modular_process_resolver"],
        field_role="module_configuration",
        module_codes=["structura_suport"],
        product_definition_keys=["mounting_solution"],
        notes=[
            "Typed support composition (metal premount / ACM cased / none).",
            "Adapter maps to process support_type; Alucobond flat is rejected.",
        ],
    ),
    IntakeFormFieldBinding(
        canonical_key="mains_cable_length_m",
        workspace_path=f"{_FINISH}.mains_cable_length_m",
        label_ro="Lungime cablu alimentare (m)",
        required=False,
        field_type="number",
        unit="m",
        option_values=["2.5", "5", "7.5", "10", "12.5", "15", "17.5", "20", "22.5", "25"],
        decision="operator_select",
        consumers=["modular_process_resolver"],
        field_role="module_configuration",
        module_codes=["sistem_led", "structura_suport"],
        product_definition_keys=["mains_cable_length_m"],
        notes=[
            "Typed process field 2.5–25 step 2.5. Not CPP in this build.",
            "Relevant for metal bars / Alucobond cased delivery paths.",
        ],
    ),
    IntakeFormFieldBinding(
        canonical_key="power_supply_service_corner",
        workspace_path=f"{_FINISH}.power_supply_service_corner",
        label_ro="Colt service transformator",
        required=False,
        field_type="enum",
        option_values=["TOP_LEFT", "TOP_RIGHT", "BOTTOM_LEFT", "BOTTOM_RIGHT", "MANUAL_CONFIRMED"],
        decision="operator_select",
        consumers=["modular_process_resolver"],
        field_role="module_configuration",
        module_codes=["structura_suport"],
        product_definition_keys=["power_supply_service_corner"],
        notes=["Required only when support maps to Alucobond cased panel."],
    ),
    IntakeFormFieldBinding(
        canonical_key="service_screw_finish",
        workspace_path=f"{_FINISH}.service_screw_finish",
        label_ro="Finisaj șuruburi corp",
        required=False,
        field_type="enum",
        option_values=["NATURAL", "PAINTED_TO_MATCH_CANT"],
        decision="operator_select",
        consumers=["modular_process_resolver"],
        field_role="module_configuration",
        module_codes=["ambalare_livrare_montaj", "modelare_cant"],
        product_definition_keys=["service_screw_finish"],
        notes=["NATURAL | PAINTED_TO_MATCH_CANT. Default NATURAL when unset."],
    ),
    IntakeFormFieldBinding(
