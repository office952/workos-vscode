...
REMOTE_FETCH_TIMEOUT_SECONDS = 20.0
REMOTE_FETCH_MAX_BYTES = 10 * 1024 * 1024
REMOTE_FETCH_MAX_REDIRECTS = 3
...
class InvalidRemoteFetchError(ValueError):
    """Raised when remote URL fetching is blocked by policy or safety checks."""
...
class AIHubService:
    ...
    def _normalize_host(host: str) -> str:
        # URL parser may return bracketed IPv6 host; normalize before checks.
    ...
    @classmethod
    def _assert_host_resolves_to_safe_public_ips(cls, host: str) -> None:
        try:
            infos = socket.getaddrinfo(host, None)
        except socket.gaierror as exc:
            raise InvalidRemoteFetchError(f"Remote URL host resolution failed: {host}") from exc

        if not infos:
            raise InvalidRemoteFetchError(f"Remote URL host resolution returned no addresses: {host}")
        ...
        for info in infos:
            ...
            try:
                ...
                continue
            if cls._is_forbidden_ip(ip_obj):
                raise InvalidRemoteFetchError(
                    f"Remote URL resolves to a forbidden target IP: {ip_obj}"
                )

    @classmethod
    def _assert_remote_url_allowed(cls, raw_url: str) -> str:
        url = (raw_url or "").strip()
        if not url:
            raise InvalidRemoteFetchError("Remote URL is empty.")

        parsed = urlparse(url)
        scheme = (parsed.scheme or "").lower()
        if scheme not in {"http", "https"}:
            raise InvalidRemoteFetchError("Only http/https schemes are supported for remote fetch.")

        host = parsed.hostname
        if not host:
            raise InvalidRemoteFetchError("Remote URL must include a host.")
        host = cls._normalize_host(host)

        # Strict fail-closed policy: remote fetch must be explicitly enabled and allowlisted.
        if not cls._is_truthy(os.environ.get("AIHUB_REMOTE_FETCH_ENABLED")):
            raise InvalidRemoteFetchError("Remote URL fetching is disabled by AIHub security policy.")

        allowlist_raw = os.environ.get("AIHUB_REMOTE_FETCH_ALLOWLIST", "")
        allowlist = {h.strip().lower() for h in allowlist_raw.split(",") if h.strip()}
        if not allowlist:
            raise InvalidRemoteFetchError(
                "Remote URL fetching requires a non-empty AIHUB_REMOTE_FETCH_ALLOWLIST."
            )
        if host not in allowlist:
            raise InvalidRemoteFetchError(f"Remote URL host is not allowlisted: {host}")

        if host in {"localhost", "127.0.0.1", "0.0.0.0", "::1"}:
            raise InvalidRemoteFetchError(f"Remote URL host is forbidden: {host}")
        ...
        try:
            ...
            # Host is a DNS name; resolve to ensure it does not map to forbidden ranges.
            cls._assert_host_resolves_to_safe_public_ips(host)

        return url

    @classmethod
    def _assert_local_file_reference_blocked(cls, source_ref: str) -> None:
        ...
        if lowered.startswith("file://"):
            raise InvalidAudioInputError("file:// sources are blocked by AIHub security policy.")

        parsed = urlparse(ref)
        if parsed.scheme and parsed.scheme.lower() not in {"http", "https", "data"}:
            raise InvalidAudioInputError(
                f"Unsupported source scheme '{parsed.scheme}' for AIHub input."
            )

        if ".." in ref:
            raise InvalidAudioInputError("Relative path traversal is blocked by AIHub security policy.")

        # Block absolute local paths on Unix and Windows.
        path = Path(ref).expanduser()
        if path.is_absolute():
            raise InvalidAudioInputError("Local absolute file paths are blocked by AIHub security policy.")
        if len(ref) >= 3 and ref[1] == ":" and ref[2] in {"\\", "/"}:
            raise InvalidAudioInputError("Local absolute file paths are blocked by AIHub security policy.")

    @classmethod
    async def _download_remote_bytes(cls, url: str) -> bytes:
        current_url = cls._assert_remote_url_allowed(url)
        ...
        async with httpx.AsyncClient(
            ...
            for _ in range(REMOTE_FETCH_MAX_REDIRECTS + 1):
                response = await client.get(current_url)

                # Validate redirect chains explicitly to prevent SSRF redirect bypass.
                if response.status_code in {301, 302, 303, 307, 308}:
                    location = response.headers.get("location", "").strip()
                    if not location:
                        raise InvalidRemoteFetchError("Remote URL redirect missing location header.")
                    next_url = urljoin(current_url, location)
                    current_url = cls._assert_remote_url_allowed(next_url)
                    continue
                ...
                async for chunk in response.aiter_bytes():
                    ...
                    if total > REMOTE_FETCH_MAX_BYTES:
                        raise InvalidRemoteFetchError(
                            f"Remote download exceeded size limit ({REMOTE_FETCH_MAX_BYTES} bytes)."
                        )
                    chunks.append(chunk)
                return b"".join(chunks)

        raise InvalidRemoteFetchError(
            f"Remote URL exceeded maximum redirect chain ({REMOTE_FETCH_MAX_REDIRECTS})."
        )
    ...
    async def _image_str_to_upload_file(self, image: str, name_prefix: str = "image") -> io.BytesIO:
        ...
        if image.startswith(("http://", "https://")):
            try:
                image_bytes = await self._download_remote_bytes(image)
    ...
    async def _audio_str_to_upload_file(self, audio: str, name_prefix: str = "audio") -> io.BytesIO:
        """
        Convert audio input (base64 data URI or policy-allowed HTTP URL) into an in-memory file object.
        ...
        self._assert_local_file_reference_blocked(audio)

        if audio.startswith(("http://", "https://")):
            try:
                audio_bytes = await self._download_remote_bytes(audio)
        ...
        raise InvalidAudioInputError(
            "Only base64 data URI or policy-allowed http(s) URL is supported for audio input. "
            "Local filesystem reads are blocked by AIHub security policy."
        )
