"""Intake V4 workspace persistence — own payload contract, no V3 form coupling."""
...
from services.intake_v4_analysis_bundle_guard_service import assert_analysis_bundle_child_parts_or_raise
...
def _workspace_code() -> str:
    return f"IV4-{uuid.uuid4().hex[:8].upper()}"


def _parse_payload(raw: dict[str, Any]) -> IntakeV4WorkspacePayload:
    return IntakeV4WorkspacePayload.model_validate(raw)


def _layer_setup_from_payload(raw: dict[str, Any]) -> IntakeV4LayerRoleSetup | None:
...
def _record_to_response(record: IntakeV4WorkspaceRecord) -> IntakeV4WorkspaceResponse:
    return IntakeV4WorkspaceResponse(
        id=record.id,
...
async def _get_record_or_404(db: AsyncSession, workspace_id: str) -> IntakeV4WorkspaceRecord:
...
async def create_intake_v4_workspace(
    ...
    now = _utcnow()
    payload = IntakeV4WorkspacePayload(
        schema_version=INTAKE_V4_SCHEMA_VERSION,
...
async def get_intake_v4_workspace(db: AsyncSession, workspace_id: str) -> IntakeV4WorkspaceResponse:
    record = await _get_record_or_404(db, workspace_id)
    return _record_to_response(record)


async def list_intake_v4_workspaces(
    db: AsyncSession,
    *,
    include_archived: bool = False,
) -> IntakeV4WorkspaceListResponse:
    query = select(IntakeV4WorkspaceRecord).order_by(IntakeV4WorkspaceRecord.updated_at.desc())
    if not include_archived:
        query = query.where(IntakeV4WorkspaceRecord.archived_at.is_(None))
    result = await db.execute(query)
    records = list(result.scalars().all())
    items = [_record_to_response(record) for record in records]
    return IntakeV4WorkspaceListResponse(items=items, total=len(items))


async def upload_svg_to_intake_v4_workspace(
    db: AsyncSession,
    workspace_id: str,
    *,
    file_name: str,
    ...
    return IntakeV4SvgUploadResponse(
        workspace=response,
...
async def save_analysis_bundle_for_intake_v4_workspace(
    db: AsyncSession,
    workspace_id: str,
...
async def save_layer_roles_for_intake_v4_workspace(
    db: AsyncSession,
    workspace_id: str,
...
async def get_task_preview_for_workspace(
    ...
    template = await resolve_product_template_or_raise(db, payload.product_binding.template_code)
    return build_v4_task_preview_response(
...
async def save_finish_setup_for_intake_v4_workspace(
    db: AsyncSession,
    workspace_id: str,
    ...
    from services.intake_v4_template_option_contract_service import validate_finish_setup_against_dossier
...
async def save_sheet_footprint_override_for_intake_v4_workspace(
    ...
    from schemas.intake_v4 import IntakeV4SheetFootprintOverrideResponse
    ...
    return IntakeV4SheetFootprintOverrideResponse(
        enabled=True,
...
async def preview_reanalyze_for_intake_v4_workspace(
    db: AsyncSession,
    workspace_id: str,
    request: "IntakeV4ReanalyzePreviewRequest",
    current_user: UserResponse,
):
    from schemas.intake_v4 import IntakeV4ReanalyzePreviewResponse, IntakeV4ReanalyzePreviewSnapshot
    from services.intake_v4_reanalyze_preview_service import compare_reanalyze_preview
    from services.intake_v4_sheet_footprint_override_service import sheet_quote_override_from_payload
    ...
    def _snapshot(raw: dict[str, Any] | None) -> IntakeV4ReanalyzePreviewSnapshot | None:
        if not raw:
            return None
        return IntakeV4ReanalyzePreviewSnapshot(**raw)

    return IntakeV4ReanalyzePreviewResponse(
        workspace_id=workspace_id,
...
async def get_pricing_input_preview_for_workspace(
    ...
    payload = _parse_payload(_json_loads(record.payload_json, {}))
    assert_v4_analysis_boundary_or_raise(payload)
    return build_v4_pricing_input_preview(workspace_id=workspace_id, payload=payload)
...
async def get_production_task_dry_run_for_workspace(
    ...
    record = await _get_record_or_404(db, workspace_id)
    payload = _parse_payload(_json_loads(record.payload_json, {}))
    return build_v4_production_task_dry_run(workspace_id=workspace_id, payload=payload)
...
async def get_production_handoff_preview_for_workspace(
    ...
    if not isinstance(payload_raw, dict):
        payload_raw = {}
    payload = _parse_payload(payload_raw)
    return await build_intake_v4_production_handoff_preview(
        db,
        workspace_id,
        payload_raw,
        payload,
...
async def get_ai_informational_assist_candidate_for_workspace(
    ...
    record = await _get_record_or_404(db, workspace_id)
    payload = _parse_payload(_json_loads(record.payload_json, {}))
    return build_intake_v4_ai_informational_assist_preview(
        workspace_id=workspace_id,
        payload=payload,
    )
...
async def get_ai_semantic_classification_candidate_for_workspace(
    ...
    record = await _get_record_or_404(db, workspace_id)
    payload = _parse_payload(_json_loads(record.payload_json, {}))
    return build_intake_v4_ai_semantic_classification_preview(
        workspace_id=workspace_id,
        payload=payload,
    )


async def create_draft_quote_for_intake_v4_workspace(
    db: AsyncSession,
    workspace_id: str,
    request,
    current_user: UserResponse,
):
    from schemas.intake_v4 import IntakeV4CreateDraftQuoteRequest, IntakeV4CreateDraftQuoteResponse
    from services.intake_v4_commercial_quote_service import create_guarded_draft_quote_from_intake_v4_workspace

    if not isinstance(request, IntakeV4CreateDraftQuoteRequest):
        request = IntakeV4CreateDraftQuoteRequest.model_validate(request)
    return await create_guarded_draft_quote_from_intake_v4_workspace(
        db,
        workspace_id,
        request,
        current_user,
