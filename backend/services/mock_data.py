...
async def initialize_mock_data():
    ...
    Controlled by ENABLE_MOCK_DATA flag:
      - Default: false in production (ENVIRONMENT != "dev")
      - Set ENABLE_MOCK_DATA=true to force seeding in any environment.
      - Set ENABLE_MOCK_DATA=false to prevent seeding even in dev.
    """
    if "MGX_IGNORE_INIT_DATA" in os.environ:
        logger.info("Ignore initialize data")
        return

    # AUDIT FIX (Task 7): ENABLE_MOCK_DATA flag controls mock seeding.
    # Default false in production, true only in dev.
    environment = os.environ.get("ENVIRONMENT", "prod").lower()
    enable_mock_raw = os.environ.get("ENABLE_MOCK_DATA", "").strip().lower()
    if enable_mock_raw in ("false", "0", "no"):
        logger.info("ENABLE_MOCK_DATA=false; skipping mock data initialization")
        return
    if enable_mock_raw not in ("true", "1", "yes"):
        # Not explicitly set — default based on environment
        if environment != "dev":
            logger.info(
                "ENABLE_MOCK_DATA not set and ENVIRONMENT=%s (not dev); "
                "skipping mock data initialization",
                environment,
            )
            return
