...
class TestF7IPublicationClassification(unittest.TestCase):
    def test_owner_missing_codes_are_canonical(self) -> None:
        self.assertEqual(
            OWNER_MISSING_COMMERCIAL_RATE_CODES,
            frozenset(
                {
    ...
    def test_inventory_marks_four_gaps_missing_and_never_zero(self) -> None:
        inv = inventory_commercial_rules_for_template(PILOT_TEMPLATE)
        by_code = {str(row["canonical_rule_code"]): row for row in inv}
        for code in OWNER_MISSING_COMMERCIAL_RATE_CODES:
            self.assertIn(code, by_code)
            row = by_code[code]
            self.assertEqual(row["publication_state"], "ACTIVE_MISSING_RATE")
            self.assertIsNone(row["current_rate"])
            self.assertEqual(row["catalog_owner"], "commercial_rules_volumetric_v2")
            self.assertIsNotNone(row["owner_question"])
...
class TestF7ITemplateRecipeReferences(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_missing_owner_rates_are_fail_closed_on_template(self) -> None:
        ...
        for code in OWNER_MISSING_COMMERCIAL_RATE_CODES:
            self.assertIn(code, by_code)
            item = by_code[code]
            self.assertEqual(item.commercial_reference_status, "ACTIVE_MISSING_RATE")
            self.assertEqual(item.status, "missing")
            self.assertIsNone(item.current_value)
            self.assertIn("COMMERCIAL_RATE_MISSING", item.blockers)
            self.assertFalse(item.commercial_ready)
            self.assertFalse(item.editable)
            self.assertEqual(item.catalog_owner, "commercial_rules_volumetric_v2")

    async def test_montaj_binds_registry_without_inventing_gaps(self) -> None:
        ...
        # Material unit_cost must not appear as commercial sell for ambalare / LED / back.
        ambalare = next(r for r in recipe.recipe if r.stable_code == "ambalare")
        self.assertIsNone(ambalare.current_value)
