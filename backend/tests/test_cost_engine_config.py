...
- Only ACTIVE PRODUCTIVE employees with `cost_lunar_firma` contribute.
- Productive hours come from Company Calendar − approved leave (not stored
  `ore_productive_luna`).
- Invalid rows (productive + active, missing cost) MUST produce warnings
...
from services.company_calendar import standard_productive_hours_for_month  # noqa: E402
...
# Fixed month for deterministic calendar hours in aggregation tests.
_TEST_YEAR = 2026
_TEST_MONTH = 7
_HOURS_JULY_2026 = standard_productive_hours_for_month(_TEST_YEAR, _TEST_MONTH)
...
class TestBaseConfigAggregation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.db.reset_tables([EmployeeAttendanceEvent, Employees, RecurringPayments])
        ...
        async def _do():
            async with self._session() as db:
                return await CostEngineConfigService(db).compute_base_config(
                    year=_TEST_YEAR, month=_TEST_MONTH
                )
    ...
    def test_only_productive_active_employees_counted(self):
        async def _do():
            async with self._session() as db:
                ...
                await emp_svc.create({
                    ...
                    "cost_lunar_firma": 8000, "ore_productive_luna": None,
                })
                await emp_svc.create({
        ...
        self.assertEqual(result["total_productive_hours_month"], _HOURS_JULY_2026)
        self.assertAlmostEqual(
            result["average_labour_hour_cost"],
            round(8000 / _HOURS_JULY_2026, 4),
        )
        ...
        async def _do():
            async with self._session() as db:
                ...
                await emp_svc.create({
                    ...
                    "cost_lunar_firma": 6000, "ore_productive_luna": None,
                })
    ...
    def test_overhead_aggregation_mixes_monthly_and_annual(self):
        ...
        self.assertAlmostEqual(
            result["overhead_hour_cost"],
            round(11000.0 / _HOURS_JULY_2026, 4),
        )
