...
# ---------------------------------------------------------------------------
# Module-level literal TextClause constants (runtime hygiene sprint, 2026-05-04).
#
# These replace the previous ``text(query_str)`` inline construction in
# ``DatabaseManager._get_table_columns``. Keeping them as module constants:
#   1. Eliminates the ``text(query_*)`` dynamic-SQL scanner pattern entirely.
#   2. Guarantees the SQL body is frozen at import time — no string
#      concatenation, no f-strings, no user input anywhere near the
#      TextClause constructor.
#   3. Preserves 100% of existing behavior: the PostgreSQL branch still
#      uses a bound ``:table_name`` parameter, and the SQLite / other-dialect
#      branches still go through their own validated / escaped paths.
# ---------------------------------------------------------------------------
_PG_TABLE_COLUMNS_SQL: TextClause = text(
...
class DatabaseManager:
    ...
    async def init_db(self):
        ...
        try:
            raw_database_url = resolve_database_url()
        except ValueError as exc:
            logger.error("No database URL provided: %s", exc)
            raise
    ...
    async def create_tables(self):
        ...
        try:
            ...
            if runtime_env in {"development", "local", "dev", "test"}:
                logger.info("Starting table structure repair (development)...")
    ...
    async def _get_table_columns(self, table_name: str):
        ...
        try:
            if self.engine.dialect.name == "postgresql":
                # Use the module-level literal TextClause constant.
                # This eliminates the ``text(query_str)`` scanner pattern
                # while preserving the bound ``:table_name`` parameter.
                query = _PG_TABLE_COLUMNS_SQL
            elif self.engine.dialect.name == "sqlite":
    ...
    try:
        async with db_manager.async_session_maker() as session:
            ...
            try:
                ...
                logger.warning("Database session HTTP error: %s", e)
                raise
            except Exception as e:
