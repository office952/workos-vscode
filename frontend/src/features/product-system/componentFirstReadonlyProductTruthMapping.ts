...
export type ComponentFirstProductTruthAllowedSource =
  | "svg_suggestion"
...
export type ComponentFirstProductTruthAllowedValueState =
  | "suggested"
  | "fallback_readonly"
  | "hydrated_readonly"
  | "manual_draft"
...
export type ComponentFirstProductTruthWritePolicy = "readonly_mapping_only";

export type ComponentFirstProductTruthRuntimeLinkState =
  | "NOT_LINKED_YET"
  | "READONLY_MAPPING_ONLY"
  | "BLOCKED_PRODUCT_TRUTH_WRITE_LEAK";

export type ComponentFirstOverallMappingState =
...
export type ComponentFirstProductTruthMappingEntry = {
  ...
  allowedSources: ComponentFirstProductTruthAllowedSource[];
  allowedValueStates: ComponentFirstProductTruthAllowedValueState[];
  blockedUntil: "owner_go_and_operator_confirmation";
  mayWriteNow: false;
  writePolicy: ComponentFirstProductTruthWritePolicy;
};

export const COMPONENT_FIRST_PRODUCT_TRUTH_DEFAULT_SOURCES: ComponentFirstProductTruthAllowedSource[] = [
  "svg_suggestion",
  "manual_operator_input",
  "catalog_default_readonly",
  "hydrated_existing_value",
...
export const COMPONENT_FIRST_PRODUCT_TRUTH_DEFAULT_VALUE_STATES: ComponentFirstProductTruthAllowedValueState[] = [
  "suggested",
  "fallback_readonly",
  "hydrated_readonly",
  "manual_draft",
...
const NON_CONFIRMED_STATES: ComponentFirstProductTruthAllowedValueState[] = [
  "suggested",
  "fallback_readonly",
  "hydrated_readonly",
  "manual_draft",
...
function componentMappingEntry(
  templateCode: ComponentFirstProductTruthMappingEntry["templateCode"],
  componentRole: string,
  fieldGroup: string,
  futureProductTruthPath: string,
  truthOwner: ComponentFirstProductTruthMappingEntry["truthOwner"],
  allowedSources: ComponentFirstProductTruthAllowedSource[] = COMPONENT_FIRST_PRODUCT_TRUTH_DEFAULT_SOURCES
): ComponentFirstProductTruthMappingEntry {
  return {
    templateCode,
    componentRole,
    fieldGroup,
    ...
    truthOwner,
    allowedSources,
    allowedValueStates: [...COMPONENT_FIRST_PRODUCT_TRUTH_DEFAULT_VALUE_STATES],
    blockedUntil: "owner_go_and_operator_confirmation",
    mayWriteNow: false,
...
export const COMPONENT_FIRST_PRODUCT_TRUTH_MAPPING_CONTRACT: ComponentFirstProductTruthMappingEntry[] = [
...
export const COMPONENT_FIRST_EXPECTED_MAPPING_ENTRY_COUNT = COMPONENT_FIRST_PRODUCT_TRUTH_MAPPING_CONTRACT.length;

export type ComponentFirstProductTruthCompactPathSummary = {
  label: string;
  pathPrefix: string;
};

export const COMPONENT_FIRST_PRODUCT_TRUTH_COMPACT_PATH_SUMMARIES: ComponentFirstProductTruthCompactPathSummary[] = [
...
function entryTreatsNonConfirmedAsConfirmed(entry: ComponentFirstProductTruthMappingEntry): boolean {
...
export function validateComponentFirstProductTruthMappingContract(
  contract: readonly ComponentFirstProductTruthMappingEntry[] = COMPONENT_FIRST_PRODUCT_TRUTH_MAPPING_CONTRACT
): { valid: boolean; issues: string[] } {
  const issues: string[] = [];
  const seen = new Set<string>();

  for (const entry of contract) {
    const key = `${normalizeComponentFirstTemplateCode(entry.templateCode)}::${entry.fieldGroup}`;
  ...
  const expectedFieldGroups = new Set(
    COMPONENT_FIRST_PRODUCT_TRUTH_MAPPING_CONTRACT.map((entry) => entry.fieldGroup)
  );
...
export type ComponentFirstProductTruthMappingAssessment = {
  ...
  runtimeProductTruthLinkState: ComponentFirstProductTruthRuntimeLinkState;
  overallMappingState: ComponentFirstOverallMappingState;
  contractEntries: ComponentFirstProductTruthMappingEntry[];
  compactPathSummaries: ComponentFirstProductTruthCompactPathSummary[];
};

export function assessComponentFirstProductTruthMapping(
  formReadiness: ComponentFirstFormSystemReadinessAssessment,
  ownerSummary: ComponentFirstOwnerSummary,
  contract: readonly ComponentFirstProductTruthMappingEntry[] = COMPONENT_FIRST_PRODUCT_TRUTH_MAPPING_CONTRACT
): ComponentFirstProductTruthMappingAssessment {
  const validation = validateComponentFirstProductTruthMappingContract(contract);
  ...
  const expectedKeys = new Set(
    COMPONENT_FIRST_PRODUCT_TRUTH_MAPPING_CONTRACT.map(
      (entry) => `${normalizeComponentFirstTemplateCode(entry.templateCode)}::${entry.fieldGroup}`
    )
  );
  const actualKeys = new Set(
    contract.map((entry) => `${normalizeComponentFirstTemplateCode(entry.templateCode)}::${entry.fieldGroup}`)
  );
  const missingMappingEntries = [...expectedKeys].filter((key) => !actualKeys.has(key));

  let runtimeProductTruthLinkState: ComponentFirstProductTruthRuntimeLinkState = "NOT_LINKED_YET";
  ...
  let overallMappingState: ComponentFirstOverallMappingState;
  if (writeEnabledEntries.length > 0 || unsafeStatePolicyEntries.length > 0) {
  ...
  return {
    ...
    expectedMappingEntriesCount: COMPONENT_FIRST_EXPECTED_MAPPING_ENTRY_COUNT,
    ...
    compactPathSummaries: COMPONENT_FIRST_PRODUCT_TRUTH_COMPACT_PATH_SUMMARIES,
  };
}

export function componentFirstProductTruthMappingLabel(state: ComponentFirstOverallMappingState): string {
  return state;
}

export function componentFirstProductTruthMappingTone(state: ComponentFirstOverallMappingState): string {
  switch (state) {
...
export function componentFirstProductTruthRuntimeLinkLabel(
  state: ComponentFirstProductTruthRuntimeLinkState
): string {
  switch (state) {
    case "NOT_LINKED_YET":
...
export function getComponentFirstProductTruthMappingsForRole(
  rolePrefix: string,
  contract: readonly ComponentFirstProductTruthMappingEntry[] = COMPONENT_FIRST_PRODUCT_TRUTH_MAPPING_CONTRACT
): ComponentFirstProductTruthMappingEntry[] {
...
export function isSuggestedTreatedAsConfirmed(entry: ComponentFirstProductTruthMappingEntry): boolean {
  return (
    entry.allowedValueStates.includes("suggested") &&
