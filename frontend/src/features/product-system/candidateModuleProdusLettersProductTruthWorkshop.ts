import { CANDIDATE_MODULE_PRODUCT_TRUTH_MAPPING_CONTRACT } from "./candidateModuleProdusReadonlyProductTruthMapping";
...
function resolvePath(
  ...
  const byFieldGroup = CANDIDATE_MODULE_PRODUCT_TRUTH_MAPPING_CONTRACT.find(
  ...
  const byExactPath = CANDIDATE_MODULE_PRODUCT_TRUTH_MAPPING_CONTRACT.find(
...
export const RETURN_CANT_WORKSHOP_FIELDS: ComponentTruthField[] = [
  ...
  field({
    ...
    status: "confirmed",
    notesRo:
      "Modulul poate fi calculat separat doar dacă deține adevărul necesar. Adevărul stă pe Module produs / Product Truth pe path-ul modulului, nu pe Product Template ca motor de preț.",
    mustNotInvent: false,
  }),
  field({
...
export const CANDIDATE_MODULE_LETTERS_PRODUCT_TRUTH_WORKSHOPS: ComponentTruthWorkshop[] = [
  {
    componentCode: RETURN_CANT_CODE,
...
export function getWorkshopByComponentCode(code: string): ComponentTruthWorkshop | null {
  ...
  return (
    CANDIDATE_MODULE_LETTERS_PRODUCT_TRUTH_WORKSHOPS.find(
...
export function buildWorkshopSummary(
  workshops: ComponentTruthWorkshop[] = CANDIDATE_MODULE_LETTERS_PRODUCT_TRUTH_WORKSHOPS,
): WorkshopSummary {
