/**
 * Volum aluminiu — dedicated structure-step page (model = Vizual față).
...
import {
  ...
  type LettersVolumeCalcCard,
} from "@/features/product-system/lettersVolumeAluminumStructureDocumentation";
import { listLettersCantFinishChips } from "@/features/product-system/lettersFinishAvailabilityDisplay";
import { LettersStructurePrincipalTaskOrderPanel } from "@/features/product-system/LettersStructurePrincipalTaskOrderPanel";
import { LETTERS_STRUCTURE_STEP_VOLUM_ALUMINIU } from "@/features/product-system/lettersStructureDetailRoutes";
import { PS_SURFACE_PANEL, PS_SURFACE_INSET } from "./productSystemSurfaces";

function CalcCardIcon({ id }: { id: LettersVolumeCalcCard["id"] }) {
  if (id === "profile_consumption") {
    return <Ruler className="h-7 w-7" aria-hidden />;
  }
  return <Palette className="h-7 w-7" aria-hidden />;
}
...
export default function LettersVolumeAluminumStructureDetailPage() {
  ...
  const cantFinishes = listLettersCantFinishChips();

  return (
    <div
      className="w-full space-y-6 pb-12"
      data-testid="letters-volume-structure-detail"
    >
      <header className="flex flex-wrap items-center justify-between gap-3">
        <Link
          to={backPath}
          className="inline-flex items-center gap-1.5 rounded-md border border-wo-border-strong bg-wo-surface-raised px-3 py-2 text-[12px] font-medium text-wo-text-secondary transition-colors hover:border-sky-500/40 hover:text-sky-100"
          data-testid="letters-volume-structure-detail-back"
        >
      ...
      <section
        className="relative overflow-hidden rounded-2xl border border-sky-500/35 bg-wo-surface-raised"
        data-testid="letters-volume-structure-detail-hero"
      >
        <div
          className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_12%_0%,rgba(14,165,233,0.28),transparent_52%),radial-gradient(ellipse_at_88%_70%,rgba(56,189,248,0.10),transparent_48%)]"
          aria-hidden
        />
        <div className="relative grid gap-8 px-6 py-7 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] lg:items-end lg:px-8 lg:py-8">
          <div>
            <div className="flex flex-wrap items-center gap-3">
              <p className="font-mono text-[12px] font-bold tabular-nums tracking-[0.22em] text-sky-300/90">
                02
              </p>
              <h1 className="text-[12px] font-bold uppercase tracking-[0.22em] text-sky-300">
                {LETTERS_VOLUME_ALUMINUM_FAMILY_LABEL_RO}
              </h1>
              <span className="rounded-md border border-sky-500/40 bg-sky-500/15 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-sky-100">
                Al 0.6 mm
              </span>
            </div>
            <p
              className="mt-4 max-w-3xl text-[2rem] font-semibold leading-[1.15] tracking-tight text-wo-text-primary sm:text-[2.35rem]"
              data-testid="letters-volume-structure-detail-material"
            >
              {LETTERS_VOLUME_STRUCTURE_DISPLAY_NAME}
            </p>
            <p className="mt-3 font-mono text-[12px] text-sky-200/55">
              {LETTERS_VOLUME_ALUMINUM_SELECTOR_CODE}
            </p>
          </div>
          <p
            className="max-w-xl border-t border-sky-500/25 pt-4 text-[13px] leading-relaxed text-sky-100/80 lg:border-t-0 lg:border-l lg:pl-8 lg:pt-0"
            data-testid="letters-volume-structure-detail-role"
          >
            {LETTERS_VOLUME_DOC_ROLE_RO}
          </p>
        </div>
      ...
      <section
        ...
        data-testid="letters-volume-structure-detail-identity"
      >
        <div
          className={`${PS_SURFACE_PANEL} flex flex-col px-5 py-5`}
          data-testid="letters-volume-structure-detail-material-card"
        >
          ...
          <p className="mt-2 inline-flex w-fit rounded-md border border-sky-800/40 bg-sky-950/30 px-2 py-1 font-mono text-[10px] text-sky-200/70">
            {LETTERS_VOLUME_ALUMINUM_SELECTOR_CODE}
          </p>
          <div className="mt-auto pt-4">
            <PricingVerifyLink
              href="/inventory/pricing?q=MAT-PROFIL-LATERAL-LITERE"
              label="Verifică prețuri profil"
              testId="letters-volume-structure-detail-material-price-verify"
            />
          </div>
        </div>
        ...
        <div
          ...
          data-testid="letters-volume-structure-detail-widths-card"
        >
          <div className="flex items-center gap-2.5">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-sky-500/15 text-sky-300">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            {LETTERS_VOLUME_ALUMINUM_WIDTHS.map((entry) => (
              <div
                key={entry.id}
                className="rounded-xl border border-sky-500/25 bg-sky-500/[0.07] px-3 py-3"
                data-testid={`letters-volume-structure-detail-width-${entry.id}`}
              >
                <p className="text-[13px] font-semibold text-wo-text-primary">
                  {LETTERS_VOLUME_ALUMINUM_FAMILY_LABEL_RO} {entry.labelRo}
                </p>
                <p className="mt-1 font-mono text-[9px] text-sky-200/60">{entry.materialCode}</p>
                <div className="mt-2">
                  <MaterialPriceVerifyLink
                    materialCode={entry.materialCode}
                    testId={`letters-volume-structure-detail-width-price-${entry.id}`}
                  />
                </div>
              </div>
            ))}
        ...
        <div
          className={`${PS_SURFACE_PANEL} px-5 py-5`}
          data-testid="letters-volume-structure-detail-process-card"
        >
          <div className="flex items-center gap-2.5">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-sky-500/15 text-sky-300">
          ...
          <div className="mt-4 space-y-2.5">
            {LETTERS_VOLUME_ALUMINUM_PROCESS_STEPS.map((step, index) => (
              <div
                key={step.id}
                className="rounded-xl border border-sky-500/25 bg-sky-500/[0.07] px-3.5 py-3"
                data-testid={`letters-volume-structure-detail-process-${step.id}`}
              >
                <p className="flex items-baseline gap-2">
                  <span className="font-mono text-[18px] font-bold tabular-nums text-sky-300/90">
                    {index + 1}
                  </span>
          ...
          {cantFinishes.length > 0 ? (
            <p className="mt-3 text-[10px] leading-snug text-wo-text-muted">
              Finisaj cant: {cantFinishes.map((chip) => chip.labelRo).join(" · ")}
            </p>
          ) : null}
        </div>
      </section>
      ...
      <LettersStructurePrincipalTaskOrderPanel
        ...
        accent="sky"
        testIdPrefix="letters-volume-structure-detail"
      />

      <section className="space-y-4" data-testid="letters-volume-structure-detail-calc">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <h2 className="text-[12px] font-bold uppercase tracking-[0.16em] text-sky-300/90">
            Cum calculăm
          </h2>
          <span className="text-[11px] text-wo-text-muted">metodă owner · fără preț duplicat</span>
        </div>
        <div className="grid gap-5 xl:grid-cols-2">
          {LETTERS_VOLUME_CALC_CARDS.map((card) => (
            <article
              key={card.id}
              className="relative overflow-hidden rounded-2xl border border-sky-500/40 bg-wo-surface-raised"
              data-testid={`letters-volume-structure-detail-calc-${card.id}`}
              data-importance={card.importance}
            >
              <div
                className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_0%_0%,rgba(14,165,233,0.16),transparent_50%)]"
                aria-hidden
              />
              <div className="relative flex h-full flex-col px-6 py-6 lg:px-7 lg:py-7">
                <div className="flex items-start gap-4">
                  <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-sky-400/35 bg-sky-500/20 text-sky-100">
                ...
                <p
                  className="mt-5 rounded-xl border border-sky-500/30 bg-sky-500/10 px-4 py-3.5 font-mono text-[13px] leading-relaxed text-sky-100"
                  data-testid={`letters-volume-structure-detail-calc-${card.id}-formula`}
                >
                  {card.formulaRo}
                </p>
                ...
                <ol className="mt-5 space-y-2.5">
                  {card.stepsRo.map((step, index) => (
                    <li key={step} className="flex gap-3 text-[13px] leading-relaxed text-wo-text-secondary">
                      <span className="font-mono text-[12px] font-bold tabular-nums text-sky-300/85">
                        {index + 1}.
                      </span>
                      <span>{step}</span>
                    </li>
                ...
                <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-sky-500/20 pt-4">
                  ...
                  <PricingVerifyLink
                    ...
                    testId={`letters-volume-structure-detail-calc-${card.id}-price-verify`}
                  />
                </div>
              </div>
            </article>
      ...
      <section
        ...
        data-testid="letters-volume-structure-detail-document"
      >
        <div className="flex flex-wrap items-center gap-2 border-b border-wo-border-subtle px-6 py-4">
          <BookOpen className="h-4 w-4 text-sky-300" aria-hidden />
        ...
        <ol
          ...
          data-testid="letters-volume-structure-detail-doc-sections"
        >
          {LETTERS_VOLUME_DOC_SECTIONS.map((section, index) => (
            <li
              key={section.id}
              className="rounded-xl border border-wo-border-subtle bg-wo-surface-inset px-5 py-5"
              data-testid={`letters-volume-structure-detail-doc-${section.id}`}
            >
              <div className="flex items-baseline gap-2.5">
                <span className="font-mono text-[12px] font-bold tabular-nums text-sky-400/80">
                ...
                <ul className="mt-3.5 space-y-2">
                  {LETTERS_VOLUME_ALUMINUM_WIDTHS.map((entry) => (
                    ...
                    >
                      ...
                      <MaterialPriceVerifyLink
                        materialCode={entry.materialCode}
                        testId={`letters-volume-structure-detail-doc-width-price-${entry.id}`}
                      />
                    </li>
                  ))}
                  {(section.bulletsRo ?? [])
                    .filter(
                      (bullet) =>
                        !LETTERS_VOLUME_ALUMINUM_WIDTHS.some((entry) =>
        ...
        <div
          ...
          data-testid="letters-volume-structure-detail-doc-sources"
        >
          <p className="text-[9px] font-semibold uppercase tracking-[0.12em] text-wo-text-muted">
            Surse owner / lock
          </p>
          <ul className="mt-2 grid gap-1.5 sm:grid-cols-2">
            {LETTERS_VOLUME_DOC_SOURCES.map((source) => (
