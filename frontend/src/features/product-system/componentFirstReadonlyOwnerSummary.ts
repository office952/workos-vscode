import type { ComponentFirstCompletenessAssessment } from "./componentFirstReadonlyCompleteness";
import type { ComponentFirstContractDriftAssessment } from "./componentFirstReadonlyCompleteness";
import type { ComponentFirstDossierAlignmentAssessment } from "./componentFirstReadonlyDossierAlignment";

export type ComponentFirstOwnerStatusLevel =
  | "SAFE_READONLY"
  | "NEEDS_LIVE_ROWS"
  | "PARTIAL_LIVE_ROWS"
  | "BLOCKED";

export type ComponentFirstOwnerNextDecision =
  | "NONE_NOW"
  | "REVIEW_ONLY"
  | "OWNER_GO_REQUIRED_FOR_NEXT_STEP";

export type ComponentFirstOwnerVisibleCheck = {
  label: string;
  value: string;
};

export type ComponentFirstOwnerSummary = {
  statusTitle: string;
  statusLevel: ComponentFirstOwnerStatusLevel;
  ...
  nextOwnerDecisionNeeded: ComponentFirstOwnerNextDecision;
  ownerVisibleChecks: ComponentFirstOwnerVisibleCheck[];
};
...
export function buildComponentFirstOwnerSummary(
  completeness: ComponentFirstCompletenessAssessment,
  drift: ComponentFirstContractDriftAssessment,
  dossier: ComponentFirstDossierAlignmentAssessment
): ComponentFirstOwnerSummary {
  ...
  const baseChecks: ComponentFirstOwnerVisibleCheck[] = [
...
export function componentFirstOwnerStatusTone(level: ComponentFirstOwnerStatusLevel): string {
  switch (level) {
    case "SAFE_READONLY":
...
export const COMPONENT_FIRST_OWNER_FORBIDDEN_WORDING = [
  "ready to quote",
  "offerable",
  "active product",
