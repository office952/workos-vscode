export const CANDIDATE_MODULE_COMPOSER_TEMPLATE_CODE = "TPL-LETTERS-COMPOSER_v1";

export const CANDIDATE_MODULE_COMPONENT_TEMPLATE_CODES = [
...
export const CANDIDATE_MODULE_EXPECTED_TEMPLATE_CODES = [
  CANDIDATE_MODULE_COMPOSER_TEMPLATE_CODE,
  ...CANDIDATE_MODULE_COMPONENT_TEMPLATE_CODES,
] as const;

export const CANDIDATE_MODULE_EXPECTED_ROW_COUNT = CANDIDATE_MODULE_EXPECTED_TEMPLATE_CODES.length;

export type CandidateModuleProdusTemplateCode = (typeof CANDIDATE_MODULE_EXPECTED_TEMPLATE_CODES)[number];

export type CandidateModuleProdusSourceMode =
  | "live_seeded_inactive"
  | "partial_live_inactive"
  | "code_contract_fallback"
  | "blocked_invalid_live_state";

export type CandidateModuleProdusCompletenessAssessment = {
  sourceMode: CandidateModuleProdusSourceMode;
  foundRowCount: number;
  expectedRowCount: number;
  foundTemplateCodes: CandidateModuleProdusTemplateCode[];
  missingTemplateCodes: CandidateModuleProdusTemplateCode[];
  invalidActiveTemplateCodes: CandidateModuleProdusTemplateCode[];
};

type CandidateModuleProdusTemplateRow = {
  template_code: string;
  active?: boolean | null;
};

export function normalizeCandidateModuleProdusTemplateCode(templateCode: string | null | undefined): string {
  return String(templateCode ?? "").trim().toUpperCase();
}

export function isCandidateModuleProdusLettersTemplate(
  templateCode: string | null | undefined
): templateCode is CandidateModuleProdusTemplateCode {
  ...
  return CANDIDATE_MODULE_EXPECTED_TEMPLATE_CODES.some(
    (candidate) => normalizeCandidateModuleProdusTemplateCode(candidate) === normalized
  );
}

function isExpectedInactiveRow(template: CandidateModuleProdusTemplateRow): boolean {
  return template.active === false;
}

function isActivationLeakRow(template: CandidateModuleProdusTemplateRow): boolean {
  return template.active !== false;
}

export function assessCandidateModuleProdusLiveCompleteness(
  templates: CandidateModuleProdusTemplateRow[]
): CandidateModuleProdusCompletenessAssessment {
  const templateByCode = new Map(
    templates.map((template) => [
      normalizeCandidateModuleProdusTemplateCode(template.template_code),
      template,
    ])
  );

  const foundTemplateCodes: CandidateModuleProdusTemplateCode[] = [];
  const missingTemplateCodes: CandidateModuleProdusTemplateCode[] = [];
  const invalidActiveTemplateCodes: CandidateModuleProdusTemplateCode[] = [];

  for (const expectedCode of CANDIDATE_MODULE_EXPECTED_TEMPLATE_CODES) {
    const template = templateByCode.get(normalizeCandidateModuleProdusTemplateCode(expectedCode));
    if (!template) {
  ...
  const expectedRowCount = CANDIDATE_MODULE_EXPECTED_ROW_COUNT;

  let sourceMode: CandidateModuleProdusSourceMode;
  if (invalidActiveTemplateCodes.length > 0) {
    ...
    foundTemplateCodes.every((code) => {
      const template = templateByCode.get(normalizeCandidateModuleProdusTemplateCode(code));
...
export function candidateModuleProdusSourceLabel(sourceMode: CandidateModuleProdusSourceMode): string {
  switch (sourceMode) {
...
export function candidateModuleProdusSourceTone(sourceMode: CandidateModuleProdusSourceMode): string {
  switch (sourceMode) {
    case "live_seeded_inactive":
...
export function candidateModuleProdusSourceDescription(sourceMode: CandidateModuleProdusSourceMode): string {
  switch (sourceMode) {
    case "live_seeded_inactive":
...
export const CANDIDATE_MODULE_FAMILY_ID = "litere_component_first_candidate";
export const CANDIDATE_MODULE_FAMILY_NAME = "Litere candidate-module candidate";

export type CandidateModuleProdusDriftState =
  | "NO_DRIFT"
...
export type CandidateModuleProdusContractCheckStatus = "OK" | "WARNING" | "BLOCKED";

export type CandidateModuleProdusFallbackContractRow = {
  ...
  compositionComponentTemplateCodes?: CandidateModuleProdusTemplateCode[];
};

export const CANDIDATE_MODULE_FALLBACK_CONTRACT_FIXTURE: CandidateModuleProdusFallbackContractRow[] = [
  {
    templateCode: CANDIDATE_MODULE_COMPOSER_TEMPLATE_CODE,
    rowKind: "composer",
    familyId: CANDIDATE_MODULE_FAMILY_ID,
    familyName: CANDIDATE_MODULE_FAMILY_NAME,
    templateKind: "product_composer",
    readiness: "planned",
    active: false,
    compositionComponentTemplateCodes: [...CANDIDATE_MODULE_COMPONENT_TEMPLATE_CODES],
  },
  {
...
export type CandidateModuleProdusLiveTemplateRow = CandidateModuleProdusTemplateRow & {
...
export type CandidateModuleProdusContractDriftAssessment = {
  driftState: CandidateModuleProdusDriftState;
  contractCheckStatus: CandidateModuleProdusContractCheckStatus;
...
function isCandidateModuleProdusFamilyTemplateCode(templateCode: string): boolean {
  const normalized = normalizeCandidateModuleProdusTemplateCode(templateCode);
  if (normalized === normalizeCandidateModuleProdusTemplateCode(CANDIDATE_MODULE_COMPOSER_TEMPLATE_CODE)) {
    return true;
  }
  return normalized.startsWith("TPL-COMP-LETTER-");
}

export function validateCandidateModuleProdusFallbackContract(
  contract: readonly CandidateModuleProdusFallbackContractRow[] = CANDIDATE_MODULE_FALLBACK_CONTRACT_FIXTURE
): { valid: boolean; issues: string[] } {
  const issues: string[] = [];

  if (contract.length !== CANDIDATE_MODULE_EXPECTED_ROW_COUNT) {
    issues.push(`fallback_contract_row_count_${contract.length}_expected_${CANDIDATE_MODULE_EXPECTED_ROW_COUNT}`);
  }
  ...
  for (const row of contract) {
    const normalized = normalizeCandidateModuleProdusTemplateCode(row.templateCode);
    if (seenCodes.has(normalized)) {
      issues.push(`fallback_contract_duplicate_code_${row.templateCode}`);
    }
    seenCodes.add(normalized);

    if (!CANDIDATE_MODULE_EXPECTED_TEMPLATE_CODES.some((code) => normalizeCandidateModuleProdusTemplateCode(code) === normalized)) {
      issues.push(`fallback_contract_extra_code_${row.templateCode}`);
    }

    if (row.rowKind === "composer") {
      composerCount += 1;
      if (row.templateCode !== CANDIDATE_MODULE_COMPOSER_TEMPLATE_CODE) {
      ...
      if (composition.length !== CANDIDATE_MODULE_COMPONENT_TEMPLATE_CODES.length) {
        issues.push("fallback_contract_composer_composition_count_mismatch");
      }
      for (const componentCode of CANDIDATE_MODULE_COMPONENT_TEMPLATE_CODES) {
      ...
      componentCount += 1;
      if (!CANDIDATE_MODULE_COMPONENT_TEMPLATE_CODES.includes(row.templateCode)) {
  ...
  for (const expectedCode of CANDIDATE_MODULE_EXPECTED_TEMPLATE_CODES) {
    if (!seenCodes.has(normalizeCandidateModuleProdusTemplateCode(expectedCode))) {
...
function compareLiveRowAgainstFallbackContract(
  liveRow: CandidateModuleProdusLiveTemplateRow,
  fallbackRow: CandidateModuleProdusFallbackContractRow
): { driftIssues: string[]; metadataWarnings: string[] } {
  const driftIssues: string[] = [];
  const metadataWarnings: string[] = [];

  if (normalizeCandidateModuleProdusTemplateCode(liveRow.template_code) !== normalizeCandidateModuleProdusTemplateCode(fallbackRow.templateCode)) {
  ...
  if (fallbackRow.rowKind === "composer") {
    if (components.length === 0) {
      ...
      const expectedCompositionCodes = [...(fallbackRow.compositionComponentTemplateCodes ?? [])]
        .map((code) => normalizeCandidateModuleProdusTemplateCode(code))
        .sort();
      const normalizedLiveCodes = liveCompositionCodes.map((code) => normalizeCandidateModuleProdusTemplateCode(code)).sort();
...
function resolveContractCheckStatus(driftState: CandidateModuleProdusDriftState, metadataWarnings: string[]): CandidateModuleProdusContractCheckStatus {
...
export function assessCandidateModuleProdusContractDrift(
  liveTemplates: CandidateModuleProdusLiveTemplateRow[],
  fallbackContract: readonly CandidateModuleProdusFallbackContractRow[] = CANDIDATE_MODULE_FALLBACK_CONTRACT_FIXTURE
): CandidateModuleProdusContractDriftAssessment {
  const fallbackValidation = validateCandidateModuleProdusFallbackContract(fallbackContract);
  const completeness = assessCandidateModuleProdusLiveCompleteness(liveTemplates);
  const fallbackByCode = new Map(
    fallbackContract.map((row) => [normalizeCandidateModuleProdusTemplateCode(row.templateCode), row])
  );
  ...
  for (const liveRow of liveTemplates) {
    const normalized = normalizeCandidateModuleProdusTemplateCode(liveRow.template_code);
    const inExpectedFamily =
      isCandidateModuleProdusFamilyTemplateCode(liveRow.template_code) ||
      liveRow.family_id === CANDIDATE_MODULE_FAMILY_ID;
    const inExpectedSet = CANDIDATE_MODULE_EXPECTED_TEMPLATE_CODES.some(
  ...
  for (const expectedCode of completeness.foundTemplateCodes) {
    ...
    const fallbackRow = fallbackByCode.get(normalizeCandidateModuleProdusTemplateCode(expectedCode));
  ...
  let driftState: CandidateModuleProdusDriftState = "NO_DRIFT";
...
export function candidateModuleProdusContractCheckLabel(status: CandidateModuleProdusContractCheckStatus): string {
  return `contract check: ${status}`;
}

export function candidateModuleProdusContractCheckTone(status: CandidateModuleProdusContractCheckStatus): string {
  switch (status) {
    case "OK":
      return "border-emerald-700/40 bg-emerald-900/20 text-emerald-300";
    case "WARNING":
...
export function candidateModuleProdusDriftLabel(driftState: CandidateModuleProdusDriftState): string {
  switch (driftState) {
    case "NO_DRIFT":
      return "NO_DRIFT";
