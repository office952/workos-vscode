...
export function CandidateModuleProdusTruthWorkshopPanel() {
  const [selectedLabel, setSelectedLabel] = useState("RETURN-CANT");
  const summary = useMemo(() => buildWorkshopSummary(), []);
  const selectedWorkshop = useMemo(
    () => getWorkshopByShortLabel(selectedLabel) ?? CANDIDATE_MODULE_LETTERS_PRODUCT_TRUTH_WORKSHOPS[0],
    [selectedLabel],
  );
  ...
  return (
    ...
    >
      ...
      >
        {CANDIDATE_MODULE_LETTERS_PRODUCT_TRUTH_WORKSHOPS.map((workshop) => {
      ...
      >
        ...
        <div className="mt-3 space-y-3">
          ...
          <div data-testid="product-system-truth-workshop-questions-skeleton">
            ...
            <ul className="mt-1 space-y-1 text-[10px] text-slate-300">
              {CANDIDATE_MODULE_LETTERS_PRODUCT_TRUTH_WORKSHOPS.filter(
