...
export default function LettersFaceStructureDetailPage() {
  ...
  return (
    ...
    >
      ...
      <section
        className="relative overflow-hidden rounded-2xl border border-violet-500/35 bg-[#111827]"
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {CNC_STEPS.map((step) => (
              ...
              >
                ...
                <p className="mt-1.5 text-[13px] font-semibold text-slate-100">{step.label}</p>
                <p className="mt-1 text-[10px] text-slate-500">{step.mark}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="space-y-4" data-testid="letters-face-structure-detail-calc">
        ...
        <div className="grid gap-5 xl:grid-cols-2">
          {LETTERS_FACE_CALC_CARDS.map((card) => (
            <article
              key={card.id}
              className="relative overflow-hidden rounded-2xl border border-violet-500/40 bg-[#111827]"
                  ...
                  {LETTERS_FACE_FINISH_MATERIALS.map((entry) => (
                    ...
                    >
                      <span>
                        ...
                        <span className="font-mono text-[10px] text-slate-500">{entry.materialCode}</span>
                      </span>
                      <MaterialPriceVerifyLink
