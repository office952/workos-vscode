...
import type { ComponentFirstFormSystemReadinessAssessment } from "./componentFirstReadonlyFormSystemReadiness";
...
import {
  ...
  type ComponentFirstProductTruthMappingAssessment,
} from "./componentFirstReadonlyProductTruthMapping";

export type ComponentFirstForbiddenValueState =
  | "suggested"
  | "fallback_readonly"
  | "hydrated_readonly"
  | "manual_draft"
...
export type ComponentFirstAllowedFutureValueState = "confirmed_later";

export type ComponentFirstMissingTruthBehavior =
...
export type ComponentFirstProductDefinitionAllowedOutput =
  | "component_readiness"
  | "missing_fields"
  | "validation_warnings"
  | "technical_definition_after_confirmed_truth";

export type ComponentFirstProductDefinitionForbiddenOutput =
  | "price"
  | "quote"
  | "order"
  | "ProductAggregate"
...
export type ComponentFirstProductDefinitionRuntimeLinkState =
  | "NOT_LINKED_YET"
  | "READONLY_CONSUMPTION_CONTRACT_ONLY"
  | "BLOCKED_PRODUCT_DEFINITION_RUNTIME_LEAK";

export type ComponentFirstOverallProductDefinitionReadinessState =
...
export type ComponentFirstProductDefinitionConsumptionEntry = {
  templateCode: ComponentFirstTemplateCode | typeof COMPONENT_FIRST_COMPOSER_TEMPLATE_CODE;
  componentRole: string;
  requiredProductTruthPaths: string[];
  optionalProductTruthPaths: string[];
  forbiddenValueStates: ComponentFirstForbiddenValueState[];
  allowedFutureValueState: ComponentFirstAllowedFutureValueState;
  missingTruthBehavior: ComponentFirstMissingTruthBehavior[];
  productDefinitionAllowedOutput: ComponentFirstProductDefinitionAllowedOutput[];
  productDefinitionForbiddenOutput: ComponentFirstProductDefinitionForbiddenOutput[];
  mayActivateProductDefinitionNow: false;
};

export const COMPONENT_FIRST_PRODUCT_DEFINITION_FORBIDDEN_VALUE_STATES: ComponentFirstForbiddenValueState[] = [
  "suggested",
  "fallback_readonly",
  "hydrated_readonly",
  "manual_draft",
...
export const COMPONENT_FIRST_PRODUCT_DEFINITION_MISSING_TRUTH_BEHAVIOR: ComponentFirstMissingTruthBehavior[] = [
  "report_missing_truth",
  "produce_readiness_blocker",
  "do_not_invent",
  "do_not_price",
...
export const COMPONENT_FIRST_PRODUCT_DEFINITION_ALLOWED_OUTPUT: ComponentFirstProductDefinitionAllowedOutput[] = [
  "component_readiness",
  "missing_fields",
  "validation_warnings",
  "technical_definition_after_confirmed_truth",
...
export const COMPONENT_FIRST_PRODUCT_DEFINITION_FORBIDDEN_OUTPUT: ComponentFirstProductDefinitionForbiddenOutput[] = [
  "price",
  "quote",
  "order",
  "ProductAggregate",
...
function consumptionEntry(
  templateCode: ComponentFirstProductDefinitionConsumptionEntry["templateCode"],
  componentRole: string,
  requiredProductTruthPaths: string[],
  optionalProductTruthPaths: string[] = []
): ComponentFirstProductDefinitionConsumptionEntry {
  return {
    templateCode,
    componentRole,
    requiredProductTruthPaths,
    optionalProductTruthPaths,
    forbiddenValueStates: [...COMPONENT_FIRST_PRODUCT_DEFINITION_FORBIDDEN_VALUE_STATES],
    allowedFutureValueState: "confirmed_later",
    missingTruthBehavior: [...COMPONENT_FIRST_PRODUCT_DEFINITION_MISSING_TRUTH_BEHAVIOR],
    productDefinitionAllowedOutput: [...COMPONENT_FIRST_PRODUCT_DEFINITION_ALLOWED_OUTPUT],
    productDefinitionForbiddenOutput: [...COMPONENT_FIRST_PRODUCT_DEFINITION_FORBIDDEN_OUTPUT],
    mayActivateProductDefinitionNow: false,
  };
}

export const COMPONENT_FIRST_PRODUCT_DEFINITION_CONSUMPTION_CONTRACT: ComponentFirstProductDefinitionConsumptionEntry[] = [
  consumptionEntry(COMPONENT_FIRST_COMPOSER_TEMPLATE_CODE, "composer_orchestration", [
...
export const COMPONENT_FIRST_PRODUCT_DEFINITION_REQUIRED_PATHS_COUNT =
  COMPONENT_FIRST_PRODUCT_DEFINITION_CONSUMPTION_CONTRACT.reduce(
    (count, entry) => count + entry.requiredProductTruthPaths.length,
    0
  );

export type ComponentFirstProductDefinitionCompactPathSummary = {
  label: string;
  paths: string;
};

export const COMPONENT_FIRST_PRODUCT_DEFINITION_COMPACT_PATH_SUMMARIES: ComponentFirstProductDefinitionCompactPathSummary[] = [
  {
    label: "FACE",
...
function detectProductDefinitionRuntimeLeakSignals(liveTemplates: ComponentFirstLiveTemplateRow[]): string[] {
  const issues: string[] = [];
...
export function validateComponentFirstProductDefinitionConsumptionContract(
  contract: readonly ComponentFirstProductDefinitionConsumptionEntry[] = COMPONENT_FIRST_PRODUCT_DEFINITION_CONSUMPTION_CONTRACT
  ...
  for (const entry of contract) {
    ...
    if (entry.forbiddenValueStates.includes("confirmed_later" as ComponentFirstForbiddenValueState)) {
...
export type ComponentFirstProductDefinitionReadinessAssessment = {
  requiredPathsCount: number;
  mappedPathsCount: number;
  missingPaths: string[];
  unsafeValueStatePaths: string[];
  forbiddenOutputSignals: string[];
  runtimeProductDefinitionLinkState: ComponentFirstProductDefinitionRuntimeLinkState;
  overallProductDefinitionReadinessState: ComponentFirstOverallProductDefinitionReadinessState;
  contractEntries: ComponentFirstProductDefinitionConsumptionEntry[];
  compactPathSummaries: ComponentFirstProductDefinitionCompactPathSummary[];
};

export function assessComponentFirstProductDefinitionReadiness(
  productTruthMapping: ComponentFirstProductTruthMappingAssessment,
  formReadiness: ComponentFirstFormSystemReadinessAssessment,
  ownerSummary: ComponentFirstOwnerSummary,
  options?: {
    liveTemplates?: ComponentFirstLiveTemplateRow[];
    contract?: readonly ComponentFirstProductDefinitionConsumptionEntry[];
  }
): ComponentFirstProductDefinitionReadinessAssessment {
  const contract = options?.contract ?? COMPONENT_FIRST_PRODUCT_DEFINITION_CONSUMPTION_CONTRACT;
  const contractValidation = validateComponentFirstProductDefinitionConsumptionContract(contract);
  ...
  let runtimeProductDefinitionLinkState: ComponentFirstProductDefinitionRuntimeLinkState = "NOT_LINKED_YET";
  ...
  let overallProductDefinitionReadinessState: ComponentFirstOverallProductDefinitionReadinessState;
  ...
  return {
    ...
    compactPathSummaries: COMPONENT_FIRST_PRODUCT_DEFINITION_COMPACT_PATH_SUMMARIES,
  };
}

export function componentFirstProductDefinitionReadinessLabel(
  state: ComponentFirstOverallProductDefinitionReadinessState
): string {
  return state;
}

export function componentFirstProductDefinitionReadinessTone(
  state: ComponentFirstOverallProductDefinitionReadinessState
): string {
  switch (state) {
...
export function componentFirstProductDefinitionRuntimeLinkLabel(
  state: ComponentFirstProductDefinitionRuntimeLinkState
): string {
  switch (state) {
    case "NOT_LINKED_YET":
...
export function getComponentFirstProductDefinitionEntry(
  templateCode: ComponentFirstTemplateCode | typeof COMPONENT_FIRST_COMPOSER_TEMPLATE_CODE,
  contract: readonly ComponentFirstProductDefinitionConsumptionEntry[] = COMPONENT_FIRST_PRODUCT_DEFINITION_CONSUMPTION_CONTRACT
): ComponentFirstProductDefinitionConsumptionEntry | undefined {
  const normalized = normalizeComponentFirstTemplateCode(templateCode);
  return contract.find((entry) => normalizeComponentFirstTemplateCode(entry.templateCode) === normalized);
}

export function getComponentFirstProductDefinitionPathsForRole(
  role: string,
  contract: readonly ComponentFirstProductDefinitionConsumptionEntry[] = COMPONENT_FIRST_PRODUCT_DEFINITION_CONSUMPTION_CONTRACT
): string[] {
