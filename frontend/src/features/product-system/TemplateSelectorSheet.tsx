...
export function TemplateSelectorSheet({
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="left"
        className="w-full sm:max-w-md bg-[#0A0F1C] border-[#1E293B] text-slate-200 p-0 flex flex-col"
      >
        <SheetHeader className="px-4 pt-5 pb-3 border-b border-[#1E293B] text-left space-y-1">
          <SheetTitle className="text-slate-100 text-[15px]">Alege șablon</SheetTitle>
        ...
        <div className="px-4 py-3 border-b border-[#1E293B] space-y-3">
          <div className="flex items-center gap-2 bg-[#111827] border border-[#1E293B] rounded-lg px-3 py-2 focus-within:border-purple-500/50">
          ...
          <div className="flex flex-wrap gap-1">
            {tabs.map((t) => (
              ...
              >
                {t.label} ({t.count})
                {t.hint ? (
                  <span className="ml-1 text-[8px] text-slate-600 uppercase">{t.hint}</span>
                ) : null}
              </button>
            ))}
          </div>
