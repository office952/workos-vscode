...
describe("productSystemHonestyShell", () => {
  it("keeps Products operational and demotes peers as În dezvoltare (not product status)", () => {
    ...
    expect(PRODUCT_SYSTEM_PLANNED_BADGE_RO).toBe("În dezvoltare");
    expect(PRODUCT_SYSTEM_PLANNED_BADGE_RO).not.toBe("Planificat");
    expect(PRODUCT_SYSTEM_PLANNED_SECTION_MESSAGE).toMatch(/nu este operațională|disponibilă ulterior/i);
  });
});
