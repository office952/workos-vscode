...
describe("IntakeV6ConfirmStep", () => {
  it("shows owner-facing summary labels and keeps technical aliases collapsed", async () => {
    renderConfirmStep();
    ...
    expect(screen.getByTestId("intake-v6-confirm-pricing-sidebar")).toBeInTheDocument();
    ...
    expect(screen.queryByTestId("intake-v6-confirm-production-parts")).not.toBeInTheDocument();
    expect(screen.queryByText("Goluri / interioare")).not.toBeInTheDocument();
    expect(screen.queryByTestId("intake-v6-confirm-inner-holes")).not.toBeInTheDocument();
    ...
    expect(screen.getByTestId("intake-v6-confirm-technical-details-toggle")).toBeInTheDocument();

    fireEvent.click(screen.getByTestId("intake-v6-confirm-operator-summary-toggle"));
    await waitFor(() => {
      expect(screen.getByTestId("intake-v6-confirm-structure")).toBeInTheDocument();
    });
    expect(screen.getByTestId("intake-v6-confirm-volumetric-letters")).toHaveTextContent("10 - luminoase");
    expect(screen.getByTestId("intake-v6-confirm-emblem-count")).toHaveTextContent("1 - luminoase");
    expect(screen.getByTestId("intake-v6-confirm-layout-parts")).toHaveTextContent("11");
    expect(screen.getByTestId("intake-v6-confirm-layers")).toHaveTextContent("3");
    fireEvent.click(screen.getByTestId("intake-v6-confirm-technical-details-toggle"));
    await waitFor(() => {
    ...
    expect(screen.getByText(/Piese vectoriale de produc/)).toBeInTheDocument();
    expect(screen.getByTestId("intake-v6-confirm-work-summary-technical-no-ocr-note")).toBeInTheDocument();
  });

  it("shows ready-for-internal-draft review status and artwork warning when only review warnings exist", async () => {
    renderConfirmStep();
