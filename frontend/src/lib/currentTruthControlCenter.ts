...
export const PRESENT_SYSTEMS: PresentSystem[] = [
  {
    ...
    limitationRo:
      "Scope stabilizare: Litere + Logo + ACM. Familie ≠ Product Template ≠ Module produs ≠ Mini-modul operațional ≠ Capability. Module storage = child PT + links (MIXED). Mini-module reuse global = NOT PROVEN. Renderer pilot Letters = PARTIAL. Nu deține tarife. Fără produse tip print textil sau colantări auto ca scope implementare.",
  ...
  },
  {
    ...
    labelRo: "Product Compiler · Definiție",
    technicalName: "ProductDefinition (internal stage)",
    owner: "Product Compiler",
    purposeRo:
      "Validează, normalizează și compilează configurația concretă; țintă: compiled active module set + active-scope readiness. Concept vizibil: Product Compiler.",
    status: "PARTIAL",
    inputRo: "Răspunsuri Intake + contract Product System + offer_scope (Letters Slice 1)",
    outputRo: "Fapte de definiție (geometrie, finisaje, module, readiness pe active scope)",
    consumerRo: "Product Compiler · Graf tehnic",
    limitationRo:
      "ACTIVE-SCOPE READINESS = PROVEN FOR LETTERS SLICE 1. offer_scope → PD compile_active_scope. Nu calculează bani. Logo/ACM out of slice. Wire intern: ProductDefinition.",
    verifyRoute: "/intake-v6",
    spineOrder: 3,
  },
  {
    id: "product_aggregate",
    labelRo: "Product Compiler · Graf tehnic",
    technicalName: "ProductAggregate (internal stage)",
    owner: "Product Compiler",
    purposeRo:
      "Rezolvă adevărul tehnic (componente, cantități, operații, minute planificate) și emite măsurători comerciale non-monetare pentru CPP 7G — selected graph pe scope activ. Concept vizibil: Product Compiler.",
    status: "PARTIAL",
    inputRo: "Product Compiler · Definiție + ActiveScopeResult",
    outputRo: "Graf tehnic + măsurători comerciale (fără preț) + task_contract",
    consumerRo: "CPP 7G (măsurători), Execution Plan (Preview → Draft → Operational)",
    limitationRo:
      "ACTIVE GRAPH = PROVEN FOR LETTERS SLICE 1. filter_aggregate_by_active_scope înainte de consumers. Nu emite prețuri. Logo/ACM out of slice. Wire intern: ProductAggregate.",
    verifyRoute: "/product-system",
    spineOrder: 4,
  },
  {
    ...
    limitationRo:
      "CPP 7G + /inventory/pricing = autoritate bani. live-calc = cale suport (nu sistem first-class). BOM = proiecție Aggregate (nu sistem first-class). CostEngine = LEGACY / nu autoritate monetară. QuoteWizard = LEGACY UI. Letters Slice 1 commercial_scope proven. Legacy /entities/quotes/price (orar) retras (HTTP 410). Minutele nu sunt autoritate comercială.",
  ...
  },
  {
    ...
    limitationRo:
      "SOLD SCOPE SNAPSHOT = PROVEN FOR LETTERS SLICE 1 (active_scope_snapshot). Nu recalculează din live workspace după freeze; dual-cost debt rămâne cunoscut; V6 official dry-run fixture debt rămâne ortogonal.",
    verifyRoute: "/quotes",
    spineOrder: 6,
  },
  {
    ...
    purposeRo: "Îngheață adevărul de comandă derivat din propunerea acceptată (passthrough scope).",
    status: "CONFIRMAT",
    inputRo: "Quote Snapshot acceptat (inclusiv active_scope_snapshot)",
    outputRo: "Snapshot comandă înghețat (copie scope, fără recompile)",
    consumerRo: "ExecutionPlan",
    limitationRo:
      "Order conversion = passthrough active_scope_snapshot; fără live Intake; fără recompile. Gate imutabilitate completă rămâne deschis pe unele căi non-scope.",
    verifyRoute: "/orders",
    spineOrder: 7,
  },
  {
...
export const PRESENT_SUPPORT_SYSTEMS: PresentSystem[] = [
  {
    ...
    status: "PARTIAL",
    inputRo: "Intake V6 offer_scope · product template · hard/conditional deps (la freeze)",
    outputRo: "active/inactive runtime sets · calc prerequisites · commercial + execution scope · QuoteSnapshotActiveScope",
    consumerRo: "ProductAggregate · CPP 7G · Quote Snapshot freeze · Order passthrough · ExecutionPlan",
    limitationRo:
      "PROVEN FOR LETTERS SLICE 1 — compile + sold-scope snapshot freeze. Logo BLOCKED; ACM PARTIAL. După freeze: fără reread workspace.",
    verifyRoute: "/modules",
    spineOrder: 0,
  },
  {
  ...
  {
    id: "cost_engine_legacy",
    labelRo: "CostEngine (LEGACY)",
    technicalName: "CostEngine",
    owner: "Protected legacy costing",
    purposeRo: "Suprafață de costing protejată — nu autoritate monetară curentă.",
    status: "INACTIV",
    inputRo: "Istoric / căi legacy",
    outputRo: "Costuri non-autoritare pentru calea comercială curentă",
    consumerRo: "Nu — CPP 7G rămâne autoritatea bani",
    limitationRo:
      "LEGACY. Nu este autoritate bani. Nu crea card Level-1 spine. Autoritate comercială: CPP 7G + /inventory/pricing.",
    verifyRoute: "/governance",
    spineOrder: 0,
  },
  {
    id: "quote_wizard_legacy",
    labelRo: "QuoteWizard (LEGACY UI)",
    technicalName: "QuoteWizard",
    owner: "Legacy commercial UI",
    purposeRo: "Suprafață UI legacy de ofertare — nu spine comercial curent.",
    status: "INACTIV",
    inputRo: "Căi UI istorice",
    outputRo: "UI legacy",
    consumerRo: "Nu pe calea Intake V6 → CPP 7G",
    limitationRo: "LEGACY UI. Nu declara current commercial authority.",
    verifyRoute: "/governance",
    spineOrder: 0,
  },
];
...
export const PRESENT_OWNERSHIP_ROWS: PresentOwnershipRow[] = [
  ...
  },
  {
    ...
    domainRo: "Module produs",
    technicalAlias: "Module (product)",
    owner: "Product System",
    semanticOwnershipRo: "Unități tehnice egale sub Product Template (child PT + links)",
    writeAuthorityRo: "STORAGE_MIXED — BOM / child TPL / dossier (fără tabel ComponentTemplate)",
    readOnlyRo: "Aggregate modules / components",
    enforcementRo: "UI: Module produs — nu „Component Template”; migrare tabel = GO owner",
    status: "PARTIAL",
  },
  {
    systemId: "mini_module",
    domainRo: "Mini-modul operațional",
    technicalAlias: "Operational mini-module",
    owner: "Product System",
    semanticOwnershipRo: "Pachet operațional (registry) — separat de Module produs",
    writeAuthorityRo: "Registry code (Letters-scoped)",
    readOnlyRo: "Form contract / PD / Aggregate",
    enforcementRo: "Scope LETTERS_ONLY / SHARED_WITHIN_SIGNAGE — nu confunda cu Module produs",
    status: "PARTIAL",
  },
  {
    systemId: "capability",
    ...
    enforcementRo: "Separate de Mini-modul operațional / offerability catalog",
    status: "PARTIAL",
  },
  {
    systemId: "intake_v6",
  ...
  },
  {
    ...
    domainRo: "Product Compiler · Definiție",
    technicalAlias: "ProductDefinition (internal)",
    owner: "Product Compiler",
    semanticOwnershipRo:
    ...
    readOnlyRo: "Product Compiler · Graf (consum)",
  ...
  },
  {
    ...
    semanticOwnershipRo:
      "Intake V6 intent · compile_active_scope · QuoteSnapshotActiveScope freeze · Order passthrough",
    writeAuthorityRo: "PD compile + Quote freeze (nu Order, nu Exec)",
    readOnlyRo: "Intake selecție · Aggregate/CPP · Order/Exec consum frozen",
    enforcementRo:
      "PROVEN FOR LETTERS SLICE 1 — snapshot closure; Exec primary = frozen exclusions; legacy thin = fallback",
    status: "PARTIAL",
  },
  {
    systemId: "product_aggregate",
    domainRo: "Product Compiler · Graf tehnic",
    technicalAlias: "ProductAggregate (internal)",
    owner: "Product Compiler",
    semanticOwnershipRo:
      "Selected technical graph · active components/materials/ops · active commercial measurements",
    writeAuthorityRo: "Compiler graph composition (sistem)",
    readOnlyRo: "CPP 7G (măsurători), Execution Plan (Preview / Draft / Operational)",
...
export const PRESENT_GATES: PresentGate[] = [
  ...
  },
  {
    ...
    approverRo: "Owner",
    enforcementRo: "Capability ≠ Mini-modul operațional; fără nume React în PS",
  ...
  },
  {
    ...
    blocksRo: "Mapă curentă: MOUNTING → {structura_suport, sablon_montaj}",
    approverRo: "Owner",
    enforcementRo: "APPROVED — fără finisaje / ambalare din MOUNTING-only",
    verificationRo: "lettersFinishMountingOwnership · offer_scope_canonical_map",
    withoutApprovalRo: "n/a — gate aprobat",
    status: "APLICAT",
  },
  {
    id: "MINI_MODULE_SPLIT_OWNER_GATE",
    nameRo: "Separare responsabilități finisaje",
    blocksRo: "finisaje / sablon_montaj / ambalare_livrare_montaj ca responsabilități precise",
    approverRo: "Owner",
    enforcementRo: "APPROVED — modul finisaje rămâne; responsabilitatea este îngustată",
