export function isValidIntakeV6SvgFile(file: File): boolean {
	const name = file.name.toLowerCase();
	if (name.endsWith(".svg")) return true;
	return file.type === "image/svg+xml";
}

export function pickIntakeV6SvgFileFromFileList(
	...
	if (files.length > 1) {
		const first = files[0];
		if (first && isValidIntakeV6SvgFile(first)) {
	...
	const file = files[0];
	if (!file || !isValidIntakeV6SvgFile(file)) {
