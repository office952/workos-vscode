...
import type { IntakeV4ArtworkComplexityDecision } from "@/lib/intakeV6/intakeV4Api";

export type { IntakeV4ArtworkComplexityDecision };
...
export function artworkComplexityDecisionsFromPayload(
  ...
  return rows.filter(
    (row): row is IntakeV4ArtworkComplexityDecision =>
      row != null &&
      typeof row === "object" &&
      typeof (row as IntakeV4ArtworkComplexityDecision).artwork_id === "string",
  );
}

export function mergeArtworkComplexityDecisions(
  existing: IntakeV4ArtworkComplexityDecision[] | null | undefined,
  assessments: ArtworkComplexityAssessment[],
): IntakeV4ArtworkComplexityDecision[] {
  const byId = new Map(
    (existing ?? []).map((row) => [row.artwork_id, row]),
  );
  const merged: IntakeV4ArtworkComplexityDecision[] = [];
...
export function decisionForArtwork(
  artworkId: string,
  decisions: IntakeV4ArtworkComplexityDecision[],
): IntakeV4ArtworkComplexityDecision | undefined {
