...
describe("intakeV6NearestOracalColor compat contract", () => {
  ...
  it("applies nearest Oracal to an explicit Oracal 651 letter group without existing code", () => {
  ...
  it("does not convert raw plexiglas selection into Oracal 651", () => {
    const next = applyNearestOracal651ToLetterGroup({
      source_fill_color: "#FF0000",
    ...
    expect(next.face_finish_type).toBe("none");
    expect(next.face_oracal_code).toBeNull();
    expect(next.face_oracal_name).toBeNull();
  });
