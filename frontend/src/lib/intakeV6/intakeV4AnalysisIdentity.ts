...
export function isLocalAnalysisSynced(state: IntakeV4WorkspaceState): boolean {
...
/**
 * After workspace hydrate/reload, align local hash with persisted svg_source.file_hash
 * unless the operator already picked a different local file (real mismatch).
 */
...
export function hasUnsavedAnalysis(state: IntakeV4WorkspaceState): boolean {
...
export function getAnalysisIdentityKey(state: IntakeV4WorkspaceState): string {
...
export function isAnalysisReadyForReview(state: IntakeV4WorkspaceState): boolean {
