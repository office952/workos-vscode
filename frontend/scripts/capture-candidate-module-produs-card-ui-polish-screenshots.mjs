/**
 * Screenshot capture for CANDIDATE_MODULE_LETTERS_CARD_UI_POLISH_AND_DRAWER_QA_FIX_V1
...
const outDir = path.resolve(
  __dirname,
  "../../docs/qa/candidate-module-card-ui-polish-2026-07-09/screenshots",
);
...
async function waitDrawer(page) {
  await page.getByTestId("product-system-candidate-module-settings-sheet").waitFor({
    state: "visible",
    timeout: 15000,
  });
  await page.getByTestId("product-system-candidate-module-readonly-drawer-banner").waitFor({
    state: "visible",
    timeout: 15000,
  });
...
async function shotDrawer(page, name) {
  ...
  const file = path.join(outDir, `${name}.png`);
  await page.locator('[data-testid="product-system-candidate-module-settings-sheet"]').screenshot({ path: file });
  console.log("saved", file);
...
async function main() {
  ...
  await page.waitForSelector('[data-testid="product-system-candidate-module-letters-set"]', { timeout: 30000 });
  ...
  await page.waitForSelector('[data-testid="product-system-candidate-module-components-list"]', { timeout: 15000 });
  ...
  await page.getByTestId("product-system-candidate-module-view-component-settings-TPL-COMP-LETTER-FACE_v1").click();
  ...
  await page.waitForSelector('[data-testid="product-system-candidate-module-dossier-workspace"]', { timeout: 15000 });
  await shotPage(page, "08_dossier_workspace");

  await page.getByTestId("product-system-candidate-module-tab-components").click();
  await page.getByTestId("product-system-candidate-module-view-dossier-TPL-COMP-LETTER-FACE_v1").click();
  await page.waitForSelector('[data-testid="product-system-candidate-module-dossier-card-TPL-COMP-LETTER-FACE_v1"][data-focused="true"]', {
    timeout: 15000,
  });
  await page
    .getByTestId("product-system-candidate-module-dossier-card-TPL-COMP-LETTER-FACE_v1")
  ...
  await page.waitForSelector('[data-testid="product-system-candidate-module-drift-guard"]', { timeout: 15000 });
